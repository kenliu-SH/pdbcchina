...
export const CurrentPage = createParamDecorator(
  (data: unknown, ctx: ExecutionContext) => {
    ...
    if (request.query) {
      let { page, pageSize, order } = request.query;
      if (request.method === 'POST') {
        if (request.body.page) page = request.body.page;
        if (request.body.pageSize) pageSize = request.body.pageSize;
        if (request.body.order) order = request.body.order;
      }
      ...
      try {
        const { page, pageSize, order } = JSON.parse(
          request.query.pagination as string
        );
