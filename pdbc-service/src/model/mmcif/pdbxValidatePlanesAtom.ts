...
@Table({ tableName: "pdbx_validate_planes_atom", timestamps: false })
export class pdbxValidatePlanesAtom extends Model {
    @Column({ field: "Structure_ID", primaryKey: true, type: DataType.STRING(10) })
    @Index({ name: "primary_index", using: "BTREE", order: "ASC", unique: true })
    structureId!: string;

    @Column({ field: "plane_id", allowNull: true, type: DataType.INTEGER })
    planeId?: number;
    ...
    @Column({ field: "atom_deviation", allowNull: true, type: DataType.FLOAT(12) })
    atomDeviation?: number;
}
