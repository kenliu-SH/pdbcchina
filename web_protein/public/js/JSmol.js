...
(function (Jmol) {
  ...
  var __nextExecution = function (trigger) {
    ...
    var es = __execStack;
    var e;
    while (es.length > 0 && (e = es[0])[4] == 'done') es.shift();
    ...
    if (self.System) System.out.println(s);
    //alert(s)
  ...
  var __loadClazz = function (applet) {
    if (!__clazzLoaded) {
      ...
      LoadClazz = null;
      if (applet.__Info.uncompressed) Clazz.loadClass(); // for now; allows for no compression
  ...
  Jmol._Canvas2D = function (id, Info, type, checkOnly) {
    ...
    if (checkOnly) return this;
    window[id] = this;
    this._createCanvas(id, Info);
    if (!Jmol._document || this._deferApplet) return this;
    this._init();
    return this;
  };

  Jmol._setAppletParams = function (
    availableParams,
    params,
    Info,
    isHashtable,
    ...
    for (var i in Info)
      if (
        ...
        if (
          Info[i] == null ||
          (i == 'language' && !Jmol.featureDetection.supportsLocalization())
        )
          continue;
        if (isHashtable)
          params.put(
  ...
  Jmol._jsSetPrototype = function (proto) {
    proto._init = function () {
      this._setupJS();
      this._showInfo(true);
      if (this._disableInitialConsole) this._showInfo(false);
    };
    ...
    proto._createCanvas = function (id, Info, glmol) {
      ...
      if (Info.addSelectionOptions) t += Jmol._getGrabberOptions(this);
      if (Jmol._debugAlert && !Jmol._document) alert(t);
      this._code = Jmol._documentWrite(t);
    };

    proto._newCanvas = function (doReplace) {
      if (this._is2D) this._createCanvas2d(doReplace);
      else this._GLmol.create();
    };
    ...
    proto._createCanvas2d = function (doReplace) {
      ...
      if (this._isLayered) {
        ...
        if (this._isSwing) {
          ...
          this._mouseInterface = this._getLayer(
            'front',
            container,
            w,
            h,
    ...
    proto._setupJS = function () {
      ...
      if (isFirst) Jmol._addExec([this, __loadClazz, null, 'loadClazz']);
      this._addCoreFiles();
    ...
    proto.__startAppletJS = function (applet) {
      if (Jmol._version.indexOf('$Date: ') == 0)
        Jmol._version =
          (Jmol._version.substring(7) + ' -').split(' -')[0] + ' (JSmol/j2s)';
      var viewerOptions = Clazz._4Name('java.util.Hashtable').newInstance();
      Jmol._setAppletParams(
        applet._availableParams,
        viewerOptions,
        applet.__Info,
        true,
      );
      ...
      if (Jmol._isAsync) viewerOptions.put('async', true);
      if (applet._color) viewerOptions.put('bgcolor', applet._color);
      ...
      if (applet._is2D) viewerOptions.put('display', applet._id + '_canvas2d');
      ...
      if (codePath.indexOf('://') < 0) {
        var base = document.location.href
          ...
          .split('?')[0]
          .split('/');
        if (codePath.indexOf('/') == 0) base = [base[0], codePath.substring(1)];
        else base[base.length - 1] = codePath;
        codePath = base.join('/');
      }
      ...
      try {
        ...
        System.out.println(
          (Jmol._isAsync ? 'normal async abort from ' : '') + e,
        );
        return;
      }
    ...
    proto._resize = function () {
      ...
      // only at end
      if (Jmol[s]) clearTimeout(Jmol[s]);
      var me = this;
      Jmol[s] = setTimeout(function () {
        Jmol.repaint(me, true);
        Jmol[s] = null;
      }, 100);
    };
  ...
  Jmol.repaint = function (applet, asNewThread) {
    ...
    var f = function () {
      if (applet._appletPanel.paint) applet._appletPanel.paint(null);
      else applet._appletPanel.update(null);
    };
    if (asNewThread) {
  ...
  Jmol.loadImage = function (platform, echoName, path, bytes, fOnload, image) {
    ...
    var canvas = Jmol.getHiddenCanvas(
      platform.vwr.html5Applet,
      id,
      0,
      0,
      false,
    ...
    if (canvas == null) {
      if (image == null) {
        ...
        if (bytes == null) {
          image.onload = function () {
            Jmol.loadImage(platform, echoName, path, null, fOnload, image);
          };
          image.src = path;
          return null;
        }
        ...
        image.src =
          typeof bytes == 'string'
            ? bytes
            : 'data:' +
              JU.Rdr.guessMimeTypeForBytes(bytes) +
              ';base64,' +
              JU.Base64.getBase64(bytes);
      }
      ...
      System.out.println(
        'Jsmol.js Jmol.loadImage reading cached image for ' + id,
      );
    }
