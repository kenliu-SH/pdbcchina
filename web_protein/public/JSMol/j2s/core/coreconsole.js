(function (
  ...
  Jmol.Console = {
    ...
    buttonWidth: 100,
    click: function (id) {
      Jmol.Console.buttons[id].console.appletConsole.doAction(
        Jmol.Console.buttons[id],
      );
    },
  };
  ...
  Jmol.Console.Image = function (vwr, title, imageMap) {
    ...
    if (!jqobj[0] && (jqobj = Jmol._$(id + '_holder'))[0]) this.id = id;
    if (jqobj[0]) this.div = jqobj;
    else
  ...
  Jmol.Console.Image.prototype.setImage = function (canvas) {
    ...
    if (this.cid) Jmol.$remove(this.cid);
  ...
  Jmol.Console.JSConsole = function (appletConsole) {
    ...
    if (userConsole[0]) {
      ...
      if (dims[0] == 0) Jmol.$setSize(userConsole, w, h);
      w = dims[0] || w;
      h = dims[1] || h;
    } else {
      s =
    ...
    s =
      setBtn(console, appletConsole.runButton) +
    ...
    if (userConsole[0]) {
      ...
      h =
        (h -
          Jmol.$getSize(id + '_label1')[1] -
          Jmol.$getSize(id + '_buttondiv')[1] -
          20) /
        3;
    } else {
      w = w - 10;
    ...
    Jmol.$bind('#' + id + '_input', 'keydown keypress keyup', function (event) {
      console.input.keyEvent(event);
    });
    Jmol.$bind('#' + id + '_input', 'mousedown touchstart', function (event) {
      console.ignoreMouse = true;
    });
    Jmol.$bind('#' + id + '_output', 'mousedown touchstart', function (event) {
      console.ignoreMouse = true;
    });
    ...
    this.keyEvent = function (ev) {
      ...
      if (type == 'keyup') {
        mode =
          kcode == 38 || kcode == 40
            ? 1
            : this.console.appletConsole.processKey(
                kcode,
                402 /*java.awt.event.KeyEvent.KEY_RELEASED*/,
                isCtrl,
              );
        if ((mode & 1) == 1) ev.preventDefault();
        return;
      }
      ...
      mode = this.console.appletConsole.processKey(
        kcode,
        401 /*java.awt.event.KeyEvent.KEY_PRESSED*/,
        isCtrl,
      );
      if (isCtrl && kcode == 10) this.setText(this.getText() + '\n');
      ...
      // ignore if...
      if (
        (mode & 1) == 1 || // Jmol has handled the key press
        key == 'Up' ||
        key == 'Down' || // up and down arrows
  ...
  Clazz_load(
    ...
    ['java.lang.Boolean', 'JU.PT', 'J.c.CBK', 'J.i18n.GT', 'JS.T', 'JV.Viewer'],
    function () {
      ...
      Clazz_defineMethod(c$, 'getLabel1', function () {
        return null;
      });
      Clazz_defineMethod(
        c$,
      ...
      Clazz_defineMethod(c$, 'setLabels', function () {
      ...
      Clazz_defineMethod(c$, 'displayConsole', function () {
        this.layoutWindow(null);
        this.outputMsg(this.defaultMessage);
      });
      Clazz_defineMethod(c$, 'updateLabels', function () {
        return;
      });
      Clazz_defineMethod(
        c$,
        ...
        function (thisCmd) {
          ...
          var strCommand =
            this.nTab <= 0 || this.incompleteCmd == null
              ? thisCmd
              : this.incompleteCmd;
          ...
          if (
            !asCommand &&
            (notThis.charAt(0) == '"' || notThis.charAt(0) == "'")
          ) {
            ...
            if (!asCommand) {
              ...
              if (inBrace || splitCmd[2].startsWith('$') || isSelect) {
                ...
                this.vwr.getObjectMap(
                  ...
                  inBrace || isSelect
                    ? '{'
                    : splitCmd[2].startsWith('$')
                    ? '$'
                    : '0',
                );
              }
            }
            cmd = JS.T.completeCommand(
              map,
              s.equalsIgnoreCase('set '),
              asCommand,
              asCommand ? splitCmd[1] : splitCmd[2],
              this.nTab,
            );
            cmd =
              splitCmd[0] +
      ...
      Clazz_defineMethod(
        ...
        function (strCommand) {
          ...
          var strErrorMessage = this.vwr.script(
            cmd + '; ## GUI ##' + '\u0001## EDITOR_IGNORE ##',
          );
      ...
      Clazz_defineMethod(c$, 'destroyConsole', function () {
        if (this.vwr.isApplet)
          this.vwr.getProperty('DATA_API', 'getAppConsole', Boolean.FALSE);
      });
      c$.setAbstractButtonLabels = Clazz_defineMethod(
        c$,
        'setAbstractButtonLabels',
        function (menuMap, labels) {
          for (
            var key, $key = menuMap.keySet().iterator();
      ...
      c$.getLabelWithoutMnemonic = Clazz_defineMethod(
        c$,
      ...
      c$.getMnemonic = Clazz_defineMethod(
        c$,
        'getMnemonic',
        function (label) {
          if (label == null) {
      ...
      Clazz_overrideMethod(
        ...
        function (type, data) {
          var strInfo =
            data == null || data[1] == null ? null : data[1].toString();
          switch (type) {
            case J.c.CBK.ECHO:
      ...
      Clazz_overrideMethod(c$, 'getText', function () {
      ...
      Clazz_overrideMethod(c$, 'zap', function () {});
      Clazz_defineMethod(
        c$,
        'recallCommand',
        function (up, pageUp) {
      ...
      c$.splitCommandLine = Clazz_defineMethod(
        c$,
        'splitCommandLine',
        function (cmd) {
          ...
          sout[1] =
            ptToken == ptCmd
              ? cmd.substring(ptCmd)
              : cmd.substring(ptCmd, ptToken > ptQ ? ptToken : ptQ);
      ...
      Clazz_defineStatics(c$, 'labels', null);
    },
  );
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'setTitle', function () {
        if (this.jsConsole != null)
      ...
      Clazz_overrideMethod(c$, 'dispose', function () {
      ...
      Clazz_overrideMethod(c$, 'getScriptEditor', function () {
        return null;
      });
      Clazz_overrideMethod(
        c$,
