(function (
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'initializeReader', function () {
        ...
        if (s != null) {
          ...
          if (pt >= 0) {
            this.setFilterAtomTypeStr(
              tokens[1].substring(pt + 1).toUpperCase(),
            );
          } else {
            pt = tokens[1].length;
          }
        ...
        this.isConnectStateBug =
          (this.stateScriptVersionInt >= 120151 &&
            this.stateScriptVersionInt <= 120220) ||
          (this.stateScriptVersionInt >= 120300 &&
            this.stateScriptVersionInt <= 120320);
      });
      Clazz_overrideMethod(c$, 'checkLine', function () {
        var ptOption =
          ((this.lineLength = this.line.length) < 6
            ? -1
      ...
      Clazz_defineMethod(c$, 'checkRemark', function () {
        this.checkCurrentLineForScript();
      });
      Clazz_defineMethod(c$, 'seqAdv', function () {
      ...
      Clazz_defineMethod(c$, 'finalizeReaderPDB', function () {
        this.checkNotPDB();
        ...
        if (this.vTlsModels != null) {
          ...
          if (n == this.vTlsModels.size()) {
            ...
            JU.Logger.info(
              n + ' models but ' + this.vTlsModels.size() + ' TLS descriptions',
            );
      ...
      Clazz_defineMethod(c$, 'checkUnitCellParams', function () {
      ...
      Clazz_defineMethod(
        ...
        function (symmetry) {
          ...
          for (
            var entry, $entry = this.tlsU.entrySet().iterator();
            ...
            entry
              .getKey()
              .addTensor(
                symmetry.getTensor(this.vwr, anisou).setType(null),
                'TLS-R',
                false,
              );
            ...
            if (anisou != null)
              JU.Logger.info('ANISOU: ' + JU.Escape.eAF(anisou));
          }
          this.tlsU = null;
        },
        'J.api.SymmetryInterface',
      );
      Clazz_defineMethod(c$, 'header', function () {
        if (this.lineLength < 8) return;
        this.appendLoadNote(this.line.substring(7).trim());
        if (this.lineLength == 80) this.maxLength = 72;
        this.pdbID =
          this.lineLength >= 66 ? this.line.substring(62, 66).trim() : '';
      ...
      Clazz_defineMethod(c$, 'title', function () {
      ...
      Clazz_defineMethod(
        ...
        function (isSource) {
          ...
          var value =
            pt < this.line.length ? this.line.substring(pt + 1).trim() : null;
          ...
          if (this.currentKey.equals('CHAIN'))
            this.currentCompnd.put(
              'select',
              '(:' + JU.PT.rep(JU.PT.rep(value, ', ', ',:'), ' ', '') + ')',
            );
        },
        '~B',
      );
      Clazz_defineMethod(c$, 'setBiomoleculeAtomCounts', function () {
        for (var i = this.vBiomolecules.size(); --i >= 0; ) {
          ...
          for (var k = nTransforms; --k >= 0; ) {
            ...
            for (var j = chains.length - 1; --j >= 0; )
              if (chains.charAt(j) == ':')
                nAtoms +=
                  this.biomtChainAtomCounts[0 + chains.charCodeAt(j + 1)];
          }
          biomolecule.put('atomCount', Integer.$valueOf(nAtoms));
        }
      });
      Clazz_defineMethod(c$, 'remark350', function () {
        var biomts = null;
        ...
        while (true) {
          ...
          try {
            if (this.line.startsWith('REMARK 350 BIOMOLECULE:')) {
              ...
              info.put(
                'molecule',
                id.length == 3 ? id : Integer.$valueOf(this.parseIntStr(id)),
              );
            ...
            if (this.line.indexOf('APPLY THE FOLLOWING TO CHAINS:') >= 0) {
              ...
              needLine = false;
              while (
                this.readHeader(true) != null &&
                this.line.indexOf('BIOMT') < 0 &&
                this.line.indexOf('350') == 7
              )
                chainlist +=
                  ':' +
                  this.line
              ...
              chainlist += ';';
              if (
                this.checkFilterKey('BIOMOLECULE ' + id + ';') ||
                this.checkFilterKey('BIOMOLECULE=' + id + ';')
              ) {
      ...
      Clazz_defineMethod(c$, 'remark285', function () {
        return true;
      });
      Clazz_defineMethod(c$, 'remark290', function () {
        while (
          this.readHeader(true) != null &&
          this.line.startsWith('REMARK 290')
        ) {
          if (this.line.indexOf('NNNMMM   OPERATOR') >= 0) {
            while (this.readHeader(true) != null) {
              var tokens = this.getTokens();
              if (tokens.length < 4) break;
              if (this.doApplySymmetry || this.isbiomol)
      ...
      Clazz_defineMethod(
        ...
        function (i, j) {
          ...
          switch (this.serMode) {
            ...
            case 2:
              return isBase10 || JU.PT.isDigit(c)
                ? this.parseIntRange(this.line, i, j)
                : JU.PT.parseIntRadix(this.line.substring(i, j), 36) +
      ...
      Clazz_defineMethod(
        ...
        function (i, j) {
          ...
          switch (this.seqMode) {
            ...
            case 1:
              if (!isBase10)
                return JU.PT.parseIntRadix(this.line.substring(i, j), 16);
              this.seqMode = 0;
              return this.getSeqNo(i, j);
          }
        },
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          if (this.biomtChainAtomCounts != null)
            this.biomtChainAtomCounts[chainID % 256]++;
      ...
      Clazz_defineMethod(
        ...
        function (atom, serial, x, y, z, charge) {
          ...
          if (atom.group3 == null) {
            ...
            !atom.group3.equals(this.currentGroup3) ||
            atom.sequenceNumber != this.currentResno
          ) {
      ...
      Clazz_defineMethod(c$, 'atom', function () {
        ...
        var atom = this.processAtom(
          ...
          this.vwr.getChainID(this.line.substring(21, 22), true),
          this.getSeqNo(22, 26),
          this.line.charAt(26),
          isHetero,
          this.deduceElementSymbol(isHetero),
        );
      ...
      Clazz_defineMethod(
        ...
        function (atom, iAtom) {
          ...
          if (this.configurationPtr > 0) {
            if (
              atom.sequenceNumber != this.lastGroup ||
              atom.insertionCode != this.lastInsertion
            ) {
            ...
            if (atom.altLoc != '\0') {
              var msg =
                ...
                (atom.chainID == 0
                  ? ''
                  : ':' + this.vwr.getChainIDStr(atom.chainID)) +
                '.' +
                atom.atomName +
                '%' +
                atom.altLoc +
      ...
      Clazz_defineMethod(
        ...
        function (atom) {
          ...
          if (this.gromacsWideFormat) {
            ...
            atom.bfactor = J.adapter.readers.pdb.PdbReader.fixRadius(
      ...
      Clazz_defineMethod(
        ...
        function (isHetero) {
          if (this.lineLength >= 78) {
            ...
            if (ch76 == ' ' && J.adapter.smarter.Atom.isValidSym1(ch77))
              return '' + ch77;
            if (J.adapter.smarter.Atom.isValidSymNoCase(ch76, ch77))
              return '' + ch76 + ch77;
          }
          ...
          if (
            (this.htElementsInCurrentGroup == null ||
              this.htElementsInCurrentGroup.get(this.line.substring(12, 14)) !=
                null) &&
            J.adapter.smarter.Atom.isValidSymNoCase(ch12, ch13)
          )
            return isHetero || ch12 != 'H' ? '' + ch12 + ch13 : 'H';
          if (ch12 == 'H') return 'H';
          if (
            (this.htElementsInCurrentGroup == null ||
              this.htElementsInCurrentGroup.get('' + ch13) != null) &&
            J.adapter.smarter.Atom.isValidSym1(ch13)
          )
            return '' + ch13;
          if (
            ch12 != ' ' &&
            (this.htElementsInCurrentGroup == null ||
              this.htElementsInCurrentGroup.get('' + ch12) != null) &&
            J.adapter.smarter.Atom.isValidSym1(ch12)
          )
            return '' + ch12;
          ...
          if (
            ...
            ch13 != 'X' &&
            (this.htElementsInCurrentGroup == null ||
              this.htElementsInCurrentGroup.get(this.line.substring(13, 15)) !=
                null) &&
            J.adapter.smarter.Atom.isValidSymNoCase(ch13, ch14)
          )
            return '' + ch13 + ch14;
          return 'Xx';
        },
      ...
      Clazz_defineMethod(c$, 'conect', function () {
        ...
        for (var pt = 11; pt < pt1; pt += 5) {
          ...
          var isDoubleBond =
            sourceSerial == this.lastSourceSerial &&
            targetSerial == this.lastTargetSerial;
      ...
      Clazz_defineMethod(c$, 'structure', function () {
        ...
        if (this.line.startsWith('HELIX ')) {
          ...
          if (this.line.length >= 40)
            substructureType = J.adapter.smarter.Structure.getHelixType(
        ...
        var startChainID = this.vwr.getChainID(
          this.line.substring(startChainIDIndex, startChainIDIndex + 1),
          true,
        );
        var startSequenceNumber = this.parseIntRange(
          this.line,
          startIndex,
          startIndex + 4,
        );
        var startInsertionCode = this.line.charAt(startIndex + 4);
        var endChainID = this.vwr.getChainID(
          this.line.substring(endChainIDIndex, endChainIDIndex + 1),
          true,
        );
        var endSequenceNumber = this.parseIntRange(
          this.line,
          endIndex,
          endIndex + 4,
        );
      ...
      Clazz_defineMethod(c$, 'getModelNumber', function () {
      ...
      Clazz_defineMethod(c$, 'getModelName', function () {
      ...
      Clazz_defineMethod(
        ...
        function (modelNumber, name) {
          ...
          this.haveMappedSerials =
            this.thisBiomolecule != null && this.applySymmetry;
          this.sbConect = null;
          this.asc.newAtomSet();
          this.asc.setCurrentModelInfo('pdbID', this.pdbID);
      ...
      Clazz_defineMethod(c$, 'checkNotPDB', function () {
        var isPDB =
          !this.isCourseGrained && (this.nRes == 0 || this.nUNK != this.nRes);
        this.asc.checkSpecial = !isPDB;
        this.setModelPDB(isPDB);
        this.nUNK = this.nRes = 0;
        this.currentGroup3 = null;
      });
      Clazz_defineMethod(c$, 'cryst1', function () {
      ...
      Clazz_defineMethod(c$, 'expdta', function () {
        if (this.line.toUpperCase().indexOf('NMR') >= 0)
          this.asc.setInfo('isNMRdata', 'true');
      });
      Clazz_defineMethod(c$, 'formul', function () {
        ...
        if (ichLeftParen >= 0) {
          ...
          if (
            ichRightParen < 0 ||
            ichLeftParen >= ichRightParen ||
            ichLeftParen + 1 == ichRightParen
          )
            return;
          formula = JU.PT.parseTrimmedRange(
            formula,
            ichLeftParen + 1,
            ichRightParen,
          );
        ...
        if (htElementsInGroup == null)
          this.htFormul.put(
            groupName,
            (htElementsInGroup = new java.util.Hashtable()),
          );
        this.next[0] = 0;
        ...
        while ((elementWithCount = this.parseTokenNext(formula)) != null) {
          ...
          else if (J.adapter.smarter.Atom.isValidSym1(chFirst))
            htElementsInGroup.put('' + chFirst, Boolean.TRUE);
        }
      });
      Clazz_defineMethod(c$, 'het', function () {
      ...
      Clazz_defineMethod(c$, 'hetnam', function () {
        if (this.htHetero == null) {
          this.htHetero = new java.util.Hashtable();
        }
      ...
      Clazz_defineMethod(c$, 'anisou', function () {
        ...
        for (var i = 28, pt = 0; i < 70; i += 7, pt++)
          data[pt] = this.parseFloatRange(this.line, i, i + 7);
      ...
      Clazz_defineMethod(c$, 'site', function () {
        if (this.htSites == null) {
      ...
      Clazz_defineMethod(c$, 'remarkTls', function () {
        var nGroups = 0;
        var iGroup = 0;
        var components = null;
        var tlsGroups = null;
        ...
        while (this.readHeader(true) != null && this.line.startsWith(remark)) {
          try {
            var tokens = JU.PT.getTokens(
              this.line.substring(10).$replace(':', ' '),
            );
            ...
            if (tokens[1].equalsIgnoreCase('GROUP')) {
              ...
              for (var i = 1; i < tokens.length; i++) {
                ...
                range.put(
                  'residues',
                  Clazz_newIntArray(-1, [resno, this.parseIntStr(tokens[++i])]),
                );
              ...
              if (tokens.length == 8) {
                origin.set(
                  this.parseFloatStr(tokens[5]),
                  this.parseFloatStr(tokens[6]),
                  this.parseFloatStr(tokens[7]),
                );
              } else {
                var n = this.line.length;
                origin.set(
                  this.parseFloatRange(this.line, n - 27, n - 18),
                  this.parseFloatRange(this.line, n - 18, n - 9),
                  this.parseFloatRange(this.line, n - 9, n),
                );
              }
              if (
              ...
              var s = (
                this.readHeader(true).substring(10) +
                this.readHeader(true).substring(10) +
                this.readHeader(true).substring(10)
              )
                .$replace(tensorType, ' ')
              ...
              for (var i = 0; i < 3; i++)
                for (var j = 0; j < 3; j++)
                  if (Float.isNaN(data[i][j]))
                    this.tlsAddError(
                      'invalid tensor: ' + JU.Escape.escapeFloatAA(data, false),
                    );
      ...
      Clazz_defineMethod(c$, 'handleTlsMissingModels', function () {
        this.vTlsModels = null;
      });
      Clazz_defineMethod(
        c$,
        ...
        function (iGroup, iModel, symmetry) {
          ...
          var data = Clazz_newFloatArray(
            this.asc.getAtomSetAtomCount(iModel),
            0,
          );
          ...
          for (var i = 0; i < nGroups; i++) {
            ...
            for (var j = ranges.size(); --j >= 0; ) {
              ...
              var res0 = residues[0];
              var res1 = residues[1];
              var index1 = this.findAtomForRange(
                index0,
                indexMax,
                chain0,
                res0,
              ...
              var index2 =
                index1 >= 0
                  ? this.findAtomForRange(index1, indexMax, chain1, res1, false)
                  : -1;
              if (index2 < 0) {
      ...
      Clazz_defineMethod(
        ...
        function (atom1, atom2, chain, resno, isLast) {
          ...
          return isLast && iAtom >= 0
            ? this.findAtom(iAtom, atom2, chain, resno, false)
            : iAtom;
        },
        '~N,~N,~N,~N,~B',
      );
      Clazz_defineMethod(
        ...
        function (atom1, atom2, chain, resno, isTrue) {
          ...
          for (var i = atom1; i < atom2; i++) {
            var atom = atoms[i];
            if (
              (atom.chainID == chain && atom.sequenceNumber == resno) == isTrue
            )
              return i;
          }
          if (isTrue) {
            JU.Logger.warn(
              'PdbReader findAtom chain=' +
                chain +
                ' resno=' +
                resno +
                ' not found',
            ...
            this.tlsAddError(
              'atom not found: chain=' + chain + ' resno=' + resno,
            );
          }
      ...
      Clazz_defineMethod(
        ...
        function (atom, group, symmetry) {
          ...
          var bresidual = Float.isNaN(atom.bfactor)
            ? 0
            : atom.bfactor / 78.95683;
          anisou[0] =
            this.dataT[0] +
            L[1][1] * zz +
            L[2][2] * yy -
          ...
          atom.addTensor(
            symmetry.getTensor(this.vwr, this.dataT).setType(null),
            'TLS-U',
            false,
          );
        },
      ...
      c$.fixRadius = Clazz_defineMethod(
        c$,
        'fixRadius',
        function (r) {
          return r < 0.9 ? 1 : r;
      ...
      Clazz_defineMethod(
        ...
        function (is) {
          ...
          if (this.connectLast != null) {
            if (
              is[0] == this.connectLast[0] &&
              is[1] == this.connectLast[1] &&
              is[2] != 2048
            ) {
              this.connectLast[2]++;
              return;
        ...
        function (maxSerial) {
          ...
          for (
            ...
            this.asc.setModelInfoForSet(
              ...
              Clazz_newIntArray(-1, [firstAtom, count, maxSerial]),
              i,
            );
            if (this.vConnect != null) {
      ...
      Clazz_defineMethod(
        ...
        function (maxSerial, isConnectStateBug) {
          ...
          a.setCurrentModelInfo(
            'PDB_CONECT_firstAtom_count_max',
            Clazz_newIntArray(-1, [
              a.getAtomSetAtomIndex(index),
              a.getAtomSetAtomCount(index),
              maxSerial,
            ]),
          );
  ...
  Clazz_load(null, 'J.adapter.smarter.Structure', ['J.c.STR'], function () {
    ...
    c$.getHelixType = Clazz_defineMethod(
      c$,
      'getHelixType',
      function (type) {
        switch (type) {
    ...
    Clazz_defineMethod(c$, 'clone', function () {
      var s = null;
      try {
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'setLeadAtomIndex', function () {
      ...
      Clazz_overrideMethod(c$, 'getSelectedMonomerCount', function () {
        return this.bioPolymer == null
          ? 0
          : this.bioPolymer.getSelectedMonomerCount();
      });
      Clazz_overrideMethod(c$, 'getSelectedMonomerIndex', function () {
        return this.bioPolymer == null ||
          !this.bioPolymer.isMonomerSelected(this.monomerIndex)
          ? -1
          : this.monomerIndex;
      });
      Clazz_overrideMethod(c$, 'getBioPolymerLength', function () {
        return this.bioPolymer == null ? 0 : this.bioPolymer.monomerCount;
      });
      Clazz_defineMethod(c$, 'getMonomerIndex', function () {
        ...
        function (name, offset) {
          if (this.bioPolymer != null) {
            ...
            if (ipt >= 0 && ipt < groups.length) {
              ...
              for (var i = m.firstAtomIndex; i <= m.lastAtomIndex; i++)
                if (
                  atoms[i] != null &&
                  (name == null ||
                    name.equalsIgnoreCase(atoms[i].getAtomName()))
                )
                  return i;
            }
          }
      ...
      Clazz_defineMethod(c$, 'getBioPolymerIndexInModel', function () {
        return this.bioPolymer == null
          ? -1
          : this.bioPolymer.bioPolymerIndexInModel;
      });
      c$.scanForOffsets = Clazz_defineMethod(
        c$,
        'scanForOffsets',
      ...
      Clazz_overrideMethod(c$, 'getProteinStructureType', function () {
        return J.c.STR.NONE;
      });
      Clazz_defineMethod(c$, 'isHelix', function () {
        return false;
      });
      Clazz_defineMethod(c$, 'isSheet', function () {
        return false;
      });
      Clazz_overrideMethod(c$, 'setStrucNo', function (id) {}, '~N');
      Clazz_defineMethod(
        c$,
        ...
        function (offsetIndex) {
          ...
          return offset == 255
            ? null
            : this.chain.model.ms.at[this.firstAtomIndex + offset];
        },
        '~N',
      );
      Clazz_defineMethod(
      ...
      Clazz_overrideMethod(c$, 'getLeadAtom', function () {
        return this.getAtomFromOffsetIndex(0);
      });
      Clazz_defineMethod(c$, 'getWingAtom', function () {
        return this.getAtomFromOffsetIndex(1);
      });
      Clazz_defineMethod(c$, 'getInitiatorAtom', function () {
        return this.getLeadAtom();
      });
      Clazz_defineMethod(c$, 'getTerminatorAtom', function () {
        return this.getLeadAtom();
      });
      Clazz_defineMethod(
        c$,
      ...
      Clazz_defineMethod(
        ...
        function (ptTemp) {
          ...
          if (Clazz_instanceOf(structure, JM.ProteinStructure)) {
            ...
            info.put(
              'structureType',
              structure.type.getBioStructureTypeName(false),
            );
          }
          info.put(
      ...
      Clazz_overrideMethod(c$, 'getStructureId', function () {
        var structure = this.getStructure();
        return Clazz_instanceOf(structure, JM.ProteinStructure)
          ? structure.type.getBioStructureTypeName(false)
          : '';
      });
      Clazz_defineMethod(
      ...
      c$.checkOptional = Clazz_defineMethod(
        c$,
        'checkOptional',
      ...
      Clazz_defineMethod(
        ...
        function (tokType, qType, mStep) {
          ...
          var prev =
            mStep < 1 || this.monomerIndex <= 0
              ? null
              : this.bioPolymer.monomers[iPrev];
          ...
          var q1 =
            mStep < 1
              ...
              : prev == null
              ? null
              : prev.getQuaternion(qType);
          if (q1 == null || q2 == null)
            return Clazz_superCall(this, JM.Monomer, 'getHelixData', [
              tokType,
              qType,
              mStep,
            ]);
          var a =
            mStep < 1
              ? JU.P3.new3(0, 0, 0)
              : prev.getQuaternionFrameCenter(qType);
          var b = this.getQuaternionFrameCenter(qType);
          return a == null || b == null
            ? this.getHelixData(tokType, qType, mStep)
            : JU.Escape.escapeHelical(
                tokType == 135176 ? 'helixaxis' + this.getUniqueID() : null,
                tokType,
                a,
                b,
                JU.Measure.computeHelicalAxis(a, b, q2.div(q1)),
              );
        },
        '~N,~S,~N',
      );
      Clazz_defineMethod(c$, 'getUniqueID', function () {
      ...
      Clazz_overrideMethod(
        ...
        function (g) {
          for (var i = this.firstAtomIndex; i <= this.lastAtomIndex; i++)
            if (this.getCrossLinkGroup(i, null, g, true, true, false))
              return true;
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          for (var j = 0; j < bonds.length; j++) {
            ...
            if (checkPrevious) {
              if (iPolymer == ibp && igroup == this.monomerIndex - 1)
                return true;
            } else if (
              iPolymer >= 0 &&
              igroup >= 0 &&
              (iPolymer != ibp ||
                igroup < this.monomerIndex - 1 ||
                igroup > this.monomerIndex + 1)
            ) {
              haveCrossLink = true;
      ...
      Clazz_defineMethod(c$, 'isConnectedPrevious', function () {
        return true;
      });
      Clazz_defineMethod(
        c$,
  ...
  Clazz_load(
    ['JM.Monomer'],
    'JM.AlphaMonomer',
    ['JU.Quat', '$.V3', 'J.c.STR', 'JM.Helix', '$.Sheet', '$.Turn'],
    function () {
      c$ = Clazz_decorateAsClass(
        function () {
      ...
      Clazz_overrideMethod(c$, 'isProtein', function () {
        return true;
      });
      c$.validateAndAllocateA = Clazz_defineMethod(
        c$,
        ...
        function (
          ...
          return firstIndex != lastIndex || specialAtomIndexes[2] != firstIndex
            ? null
      ...
      Clazz_overrideConstructor(c$, function () {});
      Clazz_defineMethod(c$, 'isAlphaMonomer', function () {
        return true;
      });
      Clazz_overrideMethod(c$, 'getStructure', function () {
      ...
      Clazz_defineMethod(
        ...
        'setStructure',
        function (ps) {
          if ((this.proteinStructure = ps) == null)
            this.nitrogenHydrogenPoint = null;
        },
        'JM.ProteinStructure',
      );
      ...
      Clazz_overrideMethod(c$, 'getProteinStructureSubType', function () {
        return this.proteinStructure == null
          ? J.c.STR.NONE
          : this.proteinStructure.subtype;
      });
      Clazz_overrideMethod(c$, 'getStrucNo', function () {
        return this.proteinStructure != null
          ? this.proteinStructure.strucNo
          : 0;
      });
      Clazz_overrideMethod(c$, 'isHelix', function () {
        return (
          this.proteinStructure != null &&
          this.proteinStructure.type === J.c.STR.HELIX
        );
      });
      Clazz_overrideMethod(c$, 'isSheet', function () {
        return (
          this.proteinStructure != null &&
      ...
      Clazz_overrideMethod(
        ...
        function (type, monomerIndexCurrent) {
          ...
          if (
            monomerIndexCurrent < 0 ||
            (monomerIndexCurrent > 0 && this.monomerIndex == 0)
          ) {
            switch (type) {
              case J.c.STR.HELIX:
              ...
              case J.c.STR.HELIXPI:
                this.setStructure(
                  new JM.Helix(this.bioPolymer, this.monomerIndex, 1, type),
                );
                break;
              case J.c.STR.SHEET:
                this.setStructure(
                  new JM.Sheet(this.bioPolymer, this.monomerIndex, 1, type),
                );
                break;
              case J.c.STR.TURN:
                this.setStructure(
            ...
            this.setStructure(
              this.bioPolymer.getProteinStructure(monomerIndexCurrent),
            );
            if (this.proteinStructure != null)
      ...
      Clazz_overrideMethod(
        ...
        function (type) {
          return (
            this.proteinStructure != null &&
            this.proteinStructure.type === type &&
        ...
        function (qType) {
          ...
          switch (qType) {
            ...
            case 'x':
              ...
              var ptCaNext = this.bioPolymer.getLeadPoint(
                this.monomerIndex + 1,
              );
              var ptCaPrev = this.bioPolymer.getLeadPoint(
                this.monomerIndex - 1,
              );
      ...
      Clazz_defineStatics(c$, 'alphaOffsets', Clazz_newByteArray(-1, [0]));
    },
  );
  Clazz_declarePackage('JM');
  Clazz_load(
    ...
    function () {
          ...
          if (JU.Logger.debugging)
            JU.Logger.info(
              'Creating ProteinStructure ' +
                ...
                ' from ' +
                apolymer.monomers[this.monomerIndexFirst] +
                ' through ' +
                apolymer.monomers[this.monomerIndexLast] +
                ' in polymer ' +
                apolymer,
            );
        },
      ...
      Clazz_defineMethod(c$, 'calcAxis', function () {});
      Clazz_defineMethod(
        c$,
        'isWithin',
        function (monomerIndex) {
          return (
            monomerIndex > this.monomerIndexFirst &&
            monomerIndex < this.monomerIndexLast
          );
        },
        '~N',
      );
      Clazz_defineMethod(
        ...
        function (monomer) {
          if (this.resMap == null) {
            ...
            for (var i = this.nRes; --i >= 0; )
              this.resMap.put(
                this.apolymer.monomers[this.monomerIndexFirst + i],
                Integer.$valueOf(i),
              );
          }
      ...
      Clazz_defineMethod(c$, 'getSegments', function () {
      ...
      Clazz_defineMethod(c$, 'calcSegments', function () {
      ...
      Clazz_defineMethod(c$, 'getAxisStartPoint', function () {
        this.calcAxis();
        return this.axisA;
      });
      Clazz_defineMethod(c$, 'getAxisEndPoint', function () {
        this.calcAxis();
        return this.axisB;
      });
      Clazz_defineMethod(c$, 'resetAxes', function () {
        this.axisA = null;
        this.segments = null;
      });
      Clazz_overrideMethod(
        ...
        function (bs) {
          ...
          for (var i = this.monomerIndexFirst; i <= this.monomerIndexLast; i++)
            ms[i].setAtomBits(bs);
        },
        'JU.BS',
      );
      ...
      Clazz_defineMethod(
        ...
        function (bsAtoms, isFirst) {
          ...
          if (isFirst) {
            for (
              var i = this.monomerIndexFirst;
              i <= this.monomerIndexLast;
              i++
            )
            ...
            for (
              var i = this.monomerIndexLast;
              i >= this.monomerIndexFirst;
              --i
            )
              if (bsAtoms == null || bsAtoms.get(ms[i].leadAtomIndex))
                return ms[i];
      ...
      Clazz_overrideMethod(c$, 'toString', function () {
        return (
          ...
          ' ' +
          this.type +
          (this.subtype == null ? '' : ' ' + this.subtype) +
          ' (' +
          this.monomerIndexFirst +
          '-' +
          this.monomerIndexLast +
      ...
      Clazz_defineStatics(c$, 'ids', 0, 'globalStrucNo', 1000);
    },
  );
  Clazz_declarePackage('JM');
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'calcAxis', function () {
        if (this.axisA != null) return;
        var points = new Array(this.nRes + 1);
        for (var i = 0; i <= this.nRes; i++)
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (i) {
          return (
            i < 0 ||
            i >= this.apolymer.monomerCount ||
            (!this.apolymer.monomers[i].isHelix() &&
              !this.apolymer.monomers[i].isSheet())
          );
        },
        '~N',
      );
      Clazz_defineMethod(c$, 'calcSheetUnitVectors', function () {
        ...
        if (this.widthUnitVector == null) {
          ...
          vectorCOSum.sub2(
            amino.getCarbonylOxygenAtom(),
            amino.getCarbonylCarbonAtom(),
          );
          for (var i = this.nRes; --i > this.monomerIndexFirst; ) {
            ...
            if (vectorCOSum.angle(vectorCO) < 1.5707964)
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'clearStructures', function () {});
      Clazz_defineMethod(c$, 'getLeadAtomIndices', function () {
      ...
      Clazz_defineMethod(
        ...
        function (chainID, seqcode, istart, iend) {
          ...
          for (i = this.monomerCount; --i >= 0; ) {
            ...
            if (
              m.chain.chainID == chainID &&
              m.seqcode == seqcode &&
              (istart < 0 ||
                istart == m.firstAtomIndex ||
                iend == m.lastAtomIndex)
            )
              break;
          }
          return i;
      ...
      Clazz_defineMethod(c$, 'getInitiatorPoint', function () {
        return this.monomers[0].getInitiatorAtom();
      });
      Clazz_defineMethod(c$, 'getTerminatorPoint', function () {
      ...
      Clazz_defineMethod(
        ...
        function (bsSelected) {
          ...
          for (var i = this.monomerCount; --i >= 0; )
            this.monomers[i].updateOffsetsForAlternativeLocations(
              atoms,
              bsSelected,
            );
      ...
      Clazz_defineMethod(c$, 'resetHydrogenPoints', function () {});
      Clazz_defineMethod(c$, 'getLeadMidpoints', function () {
        if (this.leadMidpoints == null) this.calcLeadMidpointsAndWingVectors();
        return this.leadMidpoints;
      });
      Clazz_defineMethod(c$, 'getLeadPoints', function () {
      ...
      Clazz_defineMethod(
        ...
        function (isTraceAlpha, sheetSmoothing, invalidate) {
          ...
          return !isTraceAlpha
            ...
            : sheetSmoothing == 0
            ? this.leadPoints
            : this.getControlPoints2(sheetSmoothing);
        },
        '~B,~N,~B',
      );
      Clazz_defineMethod(
        ...
        function (sheetSmoothing) {
          ...
          if (this.controlPoints == null)
            this.controlPoints = new Array(this.monomerCount + 1);
      ...
      Clazz_defineMethod(c$, 'getWingVectors', function () {
        if (this.leadMidpoints == null) this.calcLeadMidpointsAndWingVectors();
        return this.wingVectors;
      });
      Clazz_defineMethod(c$, 'calcLeadMidpointsAndWingVectors', function () {
        ...
        for (var i = 1; i < this.monomerCount; ++i) {
          ...
          if (this.hasWingPoints) {
            ...
            if (
              !this.twistedSheets &&
              previousVectorD != null &&
              previousVectorD.angle(vectorD) > 1.5707963267948966
            ) {
              this.reversed.set(i);
              vectorD.scale(-1);
            }
          ...
          if (this.monomerCount < 3) {
            ...
            for (var i = 1; i < this.monomerCount; ++i) {
              ...
              if (vectorB.length() == 0) {
                ...
                vectorC.normalize();
                if (
                  previousVectorC != null &&
                  previousVectorC.angle(vectorC) > 1.5707963267948966
                )
                  vectorC.scale(-1);
              }
      ...
      Clazz_defineMethod(c$, 'getSelectedMonomerCount', function () {
        ...
        function (last, bs, vList, isTraceAlpha, sheetSmoothing) {
          ...
          for (var j = 0; j < count; j++)
            if (bs.get(this.monomers[j].leadAtomIndex)) {
              vList.addLast(
                Clazz_newArray(-1, [points[j], JU.P3.newP(vectors[j])]),
              );
              last = j;
      ...
      Clazz_defineMethod(c$, 'getSequence', function () {
      ...
      Clazz_defineMethod(
        ...
        function (group1, nGroups, bsInclude, bsResult) {
          for (
            var i = Math.min(this.monomerCount, group1 + nGroups);
            --i >= group1;
      ...
      Clazz_defineMethod(c$, 'calcParameters', function () {
        this.haveParameters = true;
        return this.calcEtaThetaAngles() || this.calcPhiPsiAngles();
      });
      Clazz_defineMethod(c$, 'calcEtaThetaAngles', function () {
        return false;
      });
      Clazz_defineMethod(c$, 'calcPhiPsiAngles', function () {
        return false;
      });
      Clazz_defineMethod(
        c$,
      ...
      Clazz_defineMethod(c$, 'isNucleic', function () {
        return (
          this.monomerCount > 0 && Clazz_instanceOf(this, JM.NucleicPolymer)
        );
      });
      Clazz_defineMethod(
        c$,
        ...
        function (nResidues, bsAtoms, bsResult) {
          ...
          for (
            var i = bsTemp.nextSetBit(0);
            i >= 0 && i < this.monomerCount;
      ...
      Clazz_defineMethod(c$, 'getType', function () {
        return this.type;
      });
      Clazz_defineMethod(c$, 'isCyclic', function () {
        return (
          (this.cyclicFlag == 0
            ? (this.cyclicFlag =
                this.monomerCount >= 4 &&
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          if (istart < iend) {
            if (
              this.monomers[0].firstAtomIndex > iend ||
              this.monomers[this.monomerCount - 1].lastAtomIndex < istart
            )
              return;
            i0 = istart;
            i1 = iend;
          ...
          var indexEnd;
          if (
            (indexStart = this.getIndex(startChainID, startSeqcode, i0, i1)) ==
              -1 ||
            (indexEnd = this.getIndex(endChainID, endSeqcode, i0, i1)) == -1
          )
            return;
          if (istart >= 0 && bsAssigned != null) {
            var pt = bsAssigned.nextSetBit(
              this.monomers[indexStart].firstAtomIndex,
            );
      ...
      Clazz_overrideMethod(c$, 'clearStructures', function () {
        for (var i = 0; i < this.monomerCount; i++)
      ...
      Clazz_defineMethod(c$, 'calculateAnglesInDegrees', function () {
        var angles = Clazz_newFloatArray(this.monomerCount, 0);
        for (var i = this.monomerCount - 1; --i >= 2; )
          angles[i] = JU.Measure.computeTorsion(
            this.monomers[i - 2].getLeadAtom(),
            this.monomers[i - 1].getLeadAtom(),
            this.monomers[i].getLeadAtom(),
            this.monomers[i + 1].getLeadAtom(),
            true,
          );
      ...
      Clazz_defineMethod(
        ...
        function (codes, angles) {
          for (var i = this.monomerCount - 2; --i >= 2; )
            if (
              codes[i] === JM.AlphaPolymer.Code.BETA_SHEET &&
              angles[i] <= 140 &&
              codes[i - 2] === JM.AlphaPolymer.Code.RIGHT_HELIX &&
              codes[i - 1] === JM.AlphaPolymer.Code.RIGHT_HELIX &&
              codes[i + 1] === JM.AlphaPolymer.Code.RIGHT_HELIX &&
              codes[i + 2] === JM.AlphaPolymer.Code.RIGHT_HELIX
            )
              codes[i] = JM.AlphaPolymer.Code.RIGHT_HELIX;
        },
        '~A,~A',
      );
      Clazz_defineMethod(
        ...
        function (codes) {
          ...
          for (var i = 0; i < this.monomerCount; ++i) {
            if (codes[i] === code && code !== JM.AlphaPolymer.Code.NADA) {
              ...
              if (runLength == 4) {
                tag =
                  code === JM.AlphaPolymer.Code.BETA_SHEET
                    ? J.c.STR.SHEET
                    : J.c.STR.HELIX;
      ...
      Clazz_defineMethod(
        ...
        'extendRuns',
        function (tags) {
          for (var i = 1; i < this.monomerCount - 4; ++i)
            if (tags[i] === J.c.STR.NONE && tags[i + 1] !== J.c.STR.NONE)
              tags[i] = tags[i + 1];
        ...
        function (codes, angles, tags) {
          for (var i = this.monomerCount - 1; --i >= 2; ) {
            ...
            if (tags[i] == null || tags[i] === J.c.STR.NONE) {
              var angle = angles[i];
              if (angle >= -90 && angle < 0)
                codes[i] = JM.AlphaPolymer.Code.LEFT_TURN;
              else if (angle >= 0 && angle < 90)
                codes[i] = JM.AlphaPolymer.Code.RIGHT_TURN;
            }
          }
      ...
      Clazz_defineMethod(
        ...
        function (tags) {
          ...
          while (i < this.monomerCount) {
            ...
            for (
              iMax = i + 1;
              iMax < this.monomerCount && tags[iMax] === tag;
              ++iMax
            ) {}
      ...
      Clazz_defineMethod(
        ...
        function (count, dsspType, type, bs, doOffset) {
          ...
          for (
            var pt = 0,
              i = bs.nextSetBit(offset),
              i2 = 0,
              n = this.monomerCount + offset;
            i >= 0 && i < n;
            ...
            this.addStructureProtected(
              type,
              JM.AlphaPolymer.dsspTypes[dsspType] + ++pt,
              count++,
              dsspType == 3 ? 1 : 0,
              i - offset,
              i2 - 1 - offset,
      ...
      Clazz_defineStatics(
        c$,
        'dsspTypes',
        Clazz_newArray(-1, ['H', null, 'H', 'S', 'H', null, 'T']),
      );
    },
  );
  Clazz_declarePackage('JM');
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideConstructor(c$, function () {});
      c$.validateAndAllocate = Clazz_defineMethod(
        c$,
        'validateAndAllocate',
        function (
          chain,
          ...
          var offsets = JM.Monomer.scanForOffsets(
            firstAtomIndex,
            specialAtomIndexes,
            JM.AminoMonomer.interestingAminoAtomIDs,
          );
          if (offsets == null) return null;
          JM.Monomer.checkOptional(
            offsets,
            1,
            firstAtomIndex,
            specialAtomIndexes[5],
          );
          if (
            atoms[firstAtomIndex].isHetero() &&
            !JM.AminoMonomer.isBondedCorrectly(firstAtomIndex, offsets, atoms)
          )
            return null;
          return new JM.AminoMonomer().set2(
            chain,
            group3,
            seqcode,
            firstAtomIndex,
      ...
      c$.isBondedCorrectlyRange = Clazz_defineMethod(
        c$,
        ...
        function (offset1, offset2, firstAtomIndex, offsets, atoms) {
          ...
          return (
            ...
            atoms[atomIndex1].isBonded(atoms[atomIndex2])
          );
        },
        '~N,~N,~N,~A,~A',
      );
      c$.isBondedCorrectly = Clazz_defineMethod(
        c$,
        'isBondedCorrectly',
      ...
      Clazz_defineMethod(c$, 'isAminoMonomer', function () {
        return true;
      });
      Clazz_overrideMethod(c$, 'getNitrogenAtom', function () {
        return this.getAtomFromOffsetIndex(2);
      });
      Clazz_defineMethod(c$, 'getCarbonylCarbonAtom', function () {
        return this.getAtomFromOffsetIndex(3);
      });
      Clazz_overrideMethod(c$, 'getCarbonylOxygenAtom', function () {
        return this.getWingAtom();
      });
      Clazz_overrideMethod(c$, 'getInitiatorAtom', function () {
        return this.getNitrogenAtom();
      });
      Clazz_overrideMethod(c$, 'getTerminatorAtom', function () {
        return this.getAtomFromOffsetIndex(
          JM.Monomer.have(this.offsets, 4) ? 4 : 3,
        );
      });
      Clazz_defineMethod(c$, 'hasOAtom', function () {
      ...
      Clazz_overrideMethod(
        ...
        function (x, y, closest, madBegin, madEnd) {
          ...
          var radiusBegin = Clazz_floatToInt(
            this.scaleToScreen(nitrogen.sZ, marBegin),
          );
          ...
          var radiusEnd = Clazz_floatToInt(
            this.scaleToScreen(nitrogen.sZ, marEnd),
          );
          if (radiusEnd < 4) radiusEnd = 4;
          var alpha = this.getLeadAtom();
          if (
            this.isCursorOnTopOf(
              alpha,
              x,
              y,
              Clazz_doubleToInt((radiusBegin + radiusEnd) / 2),
              competitor,
            ) ||
            this.isCursorOnTopOf(nitrogen, x, y, radiusBegin, competitor) ||
            this.isCursorOnTopOf(ccarbon, x, y, radiusEnd, competitor)
          )
            closest[0] = alpha;
        },
        '~N,~N,~A,~N,~N',
      );
      Clazz_defineMethod(c$, 'resetHydrogenPoint', function () {
        this.nhChecked = false;
        this.nitrogenHydrogenPoint = null;
      });
      Clazz_defineMethod(c$, 'getNitrogenHydrogenPoint', function () {
      ...
      Clazz_defineMethod(c$, 'getExplicitNH', function () {
        var nitrogen = this.getNitrogenAtom();
        var h = null;
          ...
          if (JU.Logger.debugging)
            JU.Logger.debug(
              'draw ID "pta' +
                ...
                nitrogenPoint.i +
                '" ' +
                JU.Escape.eP(nitrogenPoint) +
                JU.Escape.eP(aminoHydrogenPoint) +
                ' # ' +
                nitrogenPoint,
            );
          return true;
      ...
      Clazz_overrideMethod(c$, 'getProteinStructureTag', function () {
        if (
          this.proteinStructure == null ||
          this.proteinStructure.structureID == null
        )
        ...
        if (this.proteinStructure.type === J.c.STR.SHEET)
          tag += JU.PT.formatStringI(
            '%2SC',
            'SC',
      ...
      Clazz_overrideMethod(c$, 'getBSSideChain', function () {
        var bs = new JU.BS();
        this.setAtomBits(bs);
      ...
      Clazz_defineStatics(
        ...
        Clazz_newByteArray(-1, [2, -5, 1, 3, -65]),
        'beta',
        0.29670597283903605,
      );
    },
  ...
  Clazz_load(
    ['JM.AlphaPolymer'],
    'JM.AminoPolymer',
    ['JU.Measure', '$.P3', '$.V3', 'J.c.STR', 'JM.HBond', 'JU.Logger'],
    function () {
      c$ = Clazz_decorateAsClass(
        function () {
      ...
      Clazz_makeConstructor(
        ...
        function (monomers, pt0) {
          ...
          this.type = 1;
          for (var i = 0; i < this.monomerCount; ++i)
            if (!monomers[i].hasOAtom()) return;

          this.hasWingPoints = true;
        },
        '~A,~N',
      );
      Clazz_overrideMethod(c$, 'resetHydrogenPoints', function () {
        var ps;
        var psLast = null;
        for (var i = 0; i < this.monomerCount; i++) {
      ...
      Clazz_overrideMethod(c$, 'calcPhiPsiAngles', function () {
        for (var i = 0; i < this.monomerCount - 1; ++i)
          this.calcPhiPsiAngles2(this.monomers[i], this.monomers[i + 1]);
      ...
      Clazz_defineMethod(
        ...
        function (residue1, residue2) {
          ...
          residue2.setGroupParameter(
            1111490569,
            JU.Measure.computeTorsion(
              carbon1,
              nitrogen2,
              alphacarbon2,
              carbon2,
      ...
      Clazz_overrideMethod(
        ...
        function (m, qtype) {
          var psiLast =
            m == 0 ? NaN : this.monomers[m - 1].getGroupParameter(1111490570);
          var psi = this.monomers[m].getGroupParameter(1111490570);
          var phi = this.monomers[m].getGroupParameter(1111490569);
          var phiNext =
            m == this.monomerCount - 1
              ? NaN
              : this.monomers[m + 1].getGroupParameter(1111490569);
          var psiNext =
            m == this.monomerCount - 1
              ? NaN
              : this.monomers[m + 1].getGroupParameter(1111490570);
          switch (qtype) {
            default:
            case 'p':
            case 'r':
            case 'P':
              ...
              return (
                ...
                2 *
                Math.acos(
                  Math.cos(dPsi) * Math.cos(dPhi) -
                    (Math.sin(dPsi) * Math.sin(dPhi)) / 3,
                )
              );
            case 'c':
            case 'C':
      ...
      Clazz_overrideMethod(
        ...
        function (
          ...
          for (var i = 1; i < this.monomerCount; ++i) {
            ...
            if (
              (source = this.monomers[i]).getNHPoint(
                pt,
                vNH,
                checkDistances,
                dsspIgnoreHydrogens,
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          for (var i = polymer.monomerCount; --i >= 0; ) {
            ...
            var energy = this.calcHbondEnergy(
              sourceNitrogenPoint,
              hydrogenPoint,
              target,
              checkDistances,
            ...
            for (var i = 0; i < 2; i++)
              if (min[i][1] >= 0)
                this.addResidueHydrogenBond(
                  nitrogen,
                  polymer.monomers[min[i][1]].getCarbonylOxygenAtom(),
                  polymer === this ? indexDonor : -99,
                  min[i][1],
                  min[i][2] / 1000,
                  vHBonds,
                );
      ...
      Clazz_defineMethod(
        ...
        function (nitrogenPoint, hydrogenPoint, target, checkDistances) {
          ...
          var isHbond =
            energy < -500 &&
            (!checkDistances || (distCN > distCH && distOH <= 3.0));
            ...
            if (JU.Logger.debugging)
              JU.Logger.debug(
                0 +
                  ...
                  i +
                  ' ' +
                  trailingResidue.getGroupParameter(1111490569) +
                  ',' +
                  leadingResidue.getGroupParameter(1111490570) +
                  ' ' +
                  structureTags[i],
              );
          }
          for (var start = 0; start < this.monomerCount; ++start) {
            if (structureTags[start] == '4') {
              ...
              for (
                ...
                end < this.monomerCount && structureTags[end] == '4';
                ++end
              ) {}
              end--;
              if (end >= start + 3) {
          ...
          for (var start = 0; start < this.monomerCount; ++start) {
            if (structureTags[start] == '3') {
              ...
              for (
                ...
                end < this.monomerCount && structureTags[end] == '3';
                ++end
              ) {}
              end--;
              if (end >= start + 3) {
          ...
          for (var start = 0; start < this.monomerCount; ++start) {
            if (structureTags[start] == 's') {
              ...
              for (
                ...
                end < this.monomerCount && structureTags[end] == 's';
                ++end
              ) {}
              end--;
              if (end >= start + 2) {
          ...
          for (var start = 0; start < this.monomerCount; ++start) {
            if (structureTags[start] == 't') {
              ...
              for (
                ...
                end < this.monomerCount && structureTags[end] == 't';
                ++end
              ) {}
              end--;
              if (end >= start + 2) {
      ...
      c$.checkPhiPsi = Clazz_defineMethod(
        c$,
        'checkPhiPsi',
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'getBioExt', function () {
        ...
        function (
          ...
          if (vHBonds == null) {
            ...
            if (bsB != null && !bsA.equals(bsB))
              (bsAtoms = JU.BSUtil.copy(bsA)).or(bsB);
            ...
            for (var i = this.ms.mc; --i >= 0; )
              if (models[i].isBioModel) models[i].hasRasmolHBonds = bsOK.get(i);

            if (bsDelete.nextSetBit(0) >= 0)
              this.ms.deleteBonds(bsDelete, false);
          }
          for (var i = this.ms.mc; --i >= 0; )
            if (am[i].isBioModel && !this.ms.isTrajectorySubFrame(i))
              am[i].getRasmolHydrogenBonds(
                bsA,
                bsB,
                vHBonds,
                nucleicOnly,
      ...
      Clazz_defineMethod(c$, 'calcSelectedMonomersCount', function () {
        var bsSelected = this.vwr.bsA();
        for (var i = this.ms.mc; --i >= 0; )
          if (this.ms.am[i].isBioModel) {
            var m = this.ms.am[i];
            for (var j = m.bioPolymerCount; --j >= 0; )
          ...
          if (modelsExcluded != null)
            for (var j = 0; j < groupCount; ++j) {
              ...
              if (Clazz_instanceOf(group, JM.Monomer)) {
                if (
                  group.bioPolymer != null &&
                  !modelsExcluded.get(group.chain.model.modelIndex)
                )
                  group.setBioPolymer(null, -1);
              }
            }
          for (var i = 0, mc = this.ms.mc; i < mc; i++)
            if (
              (modelsExcluded == null || !modelsExcluded.get(i)) &&
              this.ms.am[i].isBioModel
            ) {
              for (var pt = 0, j = baseGroupIndex; j < groupCount; ++j, pt++) {
                ...
                var doCheck =
                  ...
                  !this.ms.isJmolDataFrameForModel(
                    this.ms.at[g.getLeadAtom().i].mi,
                  );
                var bp =
                  g.bioPolymer == null
                    ? JM.BioResolver.allocateBioPolymer(groups, j, doCheck, pt)
                    : null;
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          for (
            var i = bsModels.nextSetBit(0);
            i >= 0;
            i = bsModels.nextSetBit(i + 1)
          )
            if (this.ms.am[i].isBioModel)
              ret += this.ms.am[i].calculateStructures(
                asDSSP,
                doReport,
                dsspIgnoreHydrogen,
                setStructure,
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          var bsModelsExcluded = JU.BSUtil.copyInvert(
            this.modelsOf(bsAtoms, bsAllAtoms),
            this.ms.mc,
          );
          if (!setStructure)
            return this.ms.calculateStructuresAllExcept(
              bsModelsExcluded,
              asDSSP,
              doReport,
              dsspIgnoreHydrogen,
      ...
      Clazz_defineMethod(c$, 'calculateStraightnessAll', function () {
      ...
      Clazz_defineMethod(
        ...
        'calculateStruts',
        function (bs1, bs2) {
          return this.getBioExt().calculateAllstruts(
            this.vwr,
            this.ms,
            bs1,
            bs2,
      ...
      Clazz_defineMethod(
        ...
        function (bsAtoms, bsModified) {
          ...
          for (
            ...
            if (
              this.ms.am[i].isBioModel &&
              this.ms.am[i].defaultStructure != null
            )
              ret.append(this.ms.am[i].defaultStructure);
        ...
        function (modelIndex) {
          ...
          for (var i = this.ms.mc; --i >= 0; )
            if (modelIndex < 0 || i == modelIndex) {
              ...
              ok = true;
              for (
                var entry, $entry = ht.entrySet().iterator();
                $entry.hasNext() && ((entry = $entry.next()) || true);
          ...
          for (var i = 0; i < this.ms.mc; ++i)
            if (this.ms.am[i].isBioModel) {
              ...
              for (var ip = 0; ip < m.bioPolymerCount; ip++)
                last = m.bioPolymers[ip].getPolymerPointsAndVectors(
                  last,
                  bs,
                  vList,
                  isTraceAlpha,
              ...
              if (am[i].isBioModel) {
                ...
                for (var ip = 0; ip < m.bioPolymerCount; ip++) {
                  ...
                  while ((j = sequence.indexOf(specInfo, ++j)) >= 0)
                    m.bioPolymers[ip].getPolymerSequenceAtoms(
                      j,
                      lenInfo,
                      bsAtoms,
                      bsResult,
      ...
      Clazz_defineMethod(
        ...
        function (tokType, bsInfo, bs) {
          ...
          switch (tokType) {
            case 1094713362:
              for (i = i0; i >= 0; i = bsInfo.nextSetBit(i + 1)) {
                ...
                if (iPolymer >= 0)
                  at[i].group.bioPolymer.setAtomBitsAndClear(bs, bsInfo);
              }
              break;
            case 1639976963:
              for (i = i0; i >= 0; i = bsInfo.nextSetBit(i + 1)) {
          ...
          if (i == 0)
            JU.Logger.error(
              'MISSING getAtomBits entry for ' + JS.T.nameOf(tokType),
            );
          return bs;
        },
        '~N,JU.BS,JU.BS',
          ...
          if (modelIndex < 0) {
            ...
            for (var i = this.ms.mc; --i >= 0; )
              if (!this.ms.isTrajectorySubFrame(i) && this.ms.am[i].isBioModel)
                polymerCount += this.ms.am[i].getBioPolymerCount();

            return polymerCount;
          }
          return this.ms.isTrajectorySubFrame(modelIndex) ||
            !this.ms.am[modelIndex].isBioModel
            ? 0
            : this.ms.am[modelIndex].getBioPolymerCount();
        },
        '~N',
      );
      Clazz_defineMethod(
        ...
        function (bsAtoms, mode) {
          ...
          var at0 = (
            bsAtoms == null ? this.vwr.getAllAtoms() : bsAtoms
          ).nextSetBit(0);
          if (at0 < 0) return '';
          if (bsAtoms != null && mode == 4138) {
            ...
            for (var i = this.ms.ac; --i >= 0; )
              if (
                atoms[i] == null ||
                Float.isNaN(atoms[i].group.getGroupParameter(1111490569)) ||
                Float.isNaN(atoms[i].group.getGroupParameter(1111490570))
              )
                bsAtoms.clear(i);
          }
          var at1 =
            (bsAtoms == null ? this.vwr.getAllAtoms() : bsAtoms).length() - 1;
          ...
          for (var im = im0; im <= im1; im++) {
            ...
            if (scriptMode) {
              cmd
                ...
                .append(
                  JU.Escape.eBS(
                    this.ms.getModelAtomBitSetIncludingDeleted(im, false),
                  ),
                )
                .append('    \t# model=' + this.ms.getModelNumberDotted(im))
                .append(';\n');
            }
            var ps;
            for (var i = i0; i >= 0; i = bsA.nextSetBit(i + 1)) {
              ...
              if (
                a == null ||
                !Clazz_instanceOf(a.group, JM.AlphaMonomer) ||
      ...
      Clazz_defineMethod(
        ...
        function (nResidues, bs) {
          ...
          for (var iModel = this.ms.mc; --iModel >= 0; )
            if (bsCheck.get(iModel) && this.ms.am[iModel].isBioModel) {
      ...
      Clazz_defineMethod(
        ...
        function (identifier) {
          ...
          var bs = this.ms.getSpecNameOrNull(
            identifier.substring(0, pt),
            false,
          );
          if (pt == len) return bs;
          ...
          if (pt < len && identifier.charAt(pt) == '^')
            if (++pt < len) insertionCode = identifier.charAt(pt);
          var seqcode = JM.Group.getSeqcodeFor(seqNumber, insertionCode);
          var bsInsert = this.ms.getSeqcodeBits(seqcode, false);
          if (bsInsert == null) {
            if (insertionCode != ' ')
              bsInsert = this.ms.getSeqcodeBits(
                Character.toUpperCase(identifier.charAt(pt)).charCodeAt(0),
                false,
              );
      ...
      Clazz_defineMethod(
        ...
        function (bsModelsExcluded, groups) {
          for (var i = 0; i < this.ms.mc; i++)
            if (this.ms.am[i].isBioModel && !bsModelsExcluded.get(i))
              this.ms.am[i].clearBioPolymers();
      ...
      Clazz_defineMethod(
        ...
        function (modelIndex) {
          if (modelIndex < 0) {
            for (var i = this.ms.mc; --i >= 0; )
              if (!this.ms.isTrajectorySubFrame(i) && this.ms.am[i].isBioModel)
                this.ms.am[i].recalculateLeadMidpointsAndWingVectors();

            return;
          }
          if (
            !this.ms.isTrajectorySubFrame(modelIndex) &&
            this.ms.am[modelIndex].isBioModel
          )
            this.ms.am[modelIndex].recalculateLeadMidpointsAndWingVectors();
        },
        '~N',
      );
      Clazz_defineMethod(
      ...
      Clazz_defineMethod(
        ...
        function (bs, type) {
          ...
          for (var i = 0; i < groups.length; i++) {
            ...
            monomerIndexCurrent = g.setProteinStructureType(
              type,
              monomerIndexCurrent,
            );
      ...
      Clazz_defineMethod(
        ...
        function (structureList) {
          for (var iModel = this.ms.mc; --iModel >= 0; )
            if (this.ms.am[iModel].isBioModel) {
              var m = this.ms.am[iModel];
              m.bioPolymers = JU.AU.arrayCopyObject(
                m.bioPolymers,
                m.bioPolymerCount,
              );
              for (var i = m.bioPolymerCount; --i >= 0; ) {
                var bp = m.bioPolymers[i];
                if (Clazz_instanceOf(bp, JM.AminoPolymer))
                  bp.setStructureList(structureList);
              }
            }
        },
        'java.util.Map',
        ...
        function (specInfo) {
          ...
          if (specInfo.length == 0) {
            ...
            for (var i = 0; i < specInfo.length; ) {
              bsA = this.ms.getSequenceBits(
                specInfo.substring(i, ++i),
                null,
                new JU.BS(),
              );
              if (bsA.nextSetBit(0) < 0) continue;
              bsB = this.ms.getSequenceBits(
                specInfo.substring(i, ++i),
                null,
                new JU.BS(),
              );
      ...
      Clazz_defineMethod(
        ...
        function (specInfo, bsSelected, bsResult) {
          ...
          if (maps == null) {
            ...
            for (var i = this.ms.mc; --i >= 0; ) {
              ...
              if (this.ms.isTrajectory(i))
                m = this.ms.am[(i = m.trajectoryBaseIndex)];
              var num = '|' + this.ms.getInfo(i, 'modelNumber');
              this.checkMap(
                maps[0],
                this.ms.getInfo(i, 'modelName') + num,
                m.bsAtoms,
              );
          ...
          var bsTemp = new JU.BS();
          var units = JU.PT.getTokens(
            JU.PT.replaceAllCharacters(specInfo, ', \t\n[]"=', ' '),
          );
          ...
          for (var i = units.length; --i >= 0; ) {
            ...
            var bsPtr = 0;
            for (
              var j = 0, n = 0, pt = unit.lastIndexOf('|') + 1;
              j < pt && n < 8;
              j++
            ) {
            ...
            if (lastModelChain != null && lastModelChain.equals(mchain)) {
              ...
              if (
                !this.addUnit(
                  1094717454,
                  unit.substring(0, ptrs[1]).toUpperCase(),
                  bsTemp,
                  maps[0],
                ) ||
                !this.addUnit(
                  1073742357,
                  unit.substring(ptrs[1] + 1, ptrs[2]),
                  bsTemp,
                  maps[1],
                )
              )
            ...
            if (
              !this.addUnit(
                1094715412,
                unit.substring(ptrs[3] + 1, ptrs[4]),
                bsTemp,
                maps[2],
              ) ||
              !this.addUnit(
              ...
              (haveAtom
                ? !this.addUnit(
                    1086326786,
                    unit.substring(ptrs[4] + 1, ptrs[5]).toUpperCase(),
                    bsTemp,
                    maps[4],
                  ) ||
                  !this.addUnit(
                    1073742355,
                    unit.substring(ptrs[5] + 1, ptrs[6]),
                    bsTemp,
                    maps[5],
                  )
                : haveAlt &&
      ...
      Clazz_defineMethod(
        ...
        function (name, tok, specInfo) {
          ...
          var ann;
          for (var i = this.ms.mc; --i >= 0; )
            if ((ann = this.ms.getInfo(i, name)) != null)
              bs.or(
                pa.getAtomBits(
                  this.vwr,
                  specInfo,
                  this.ms.am[i].getCachedAnnotationMap(name + ' V ', ann),
                  this.ms.am[i].dssrCache,
                  tok,
                  i,
                  this.ms.am[i].bsAtoms,
                ),
              );
      ...
      Clazz_defineMethod(
        ...
        function (bsAtoms, cmd, lstStr, type, scriptMode, mode) {
          ...
          for (var i = 0, ns = lstStr.size(); i < ns; i++) {
            ...
            var comment = scriptMode
              ? '    \t# model=' + this.ms.getModelNumberDotted(iModel)
              : null;
            ...
            switch (type) {
              ...
              case J.c.STR.SHEET:
                ...
                if (scriptMode) {
                  ...
                  str = JU.PT.formatStringS(
                    str,
                    'CA',
                    m1.getLeadAtom().getChainIDStr(),
                  );
          ...
          for (
            ...
            if (this.ms.am[i].isBioModel) {
              ...
              if (m.defaultStructure == null)
                m.defaultStructure = this.getFullProteinStructureState(
                  m.bsAtoms,
                  1073742158,
                );
            }
      ...
      Clazz_defineMethod(
        ...
        function (s, atomName, aaRet) {
          return this.getBioExt().getAminoAcidValenceAndCharge(
            s,
            atomName,
            aaRet,
          );
      ...
      Clazz_defineStatics(c$, 'DEFAULT_DSSP_VERSION', 2);
    },
  );
  Clazz_declarePackage('JM');
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          for (var i = this.bioPolymerCount; --i >= 0; )
            if (Clazz_instanceOf(this.bioPolymers[i], JM.AlphaPolymer))
              this.bioPolymers[i].addStructure(
                type,
                structureID,
                serialID,
                strandCount,
      ...
      Clazz_defineMethod(
        ...
        function (count, dsspType, type, bs) {
          for (var i = this.bioPolymerCount; --i >= 0; ) {
            ...
            if (Clazz_instanceOf(b, JM.AlphaPolymer))
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          for (
            var i = 0;
            i < this.bioPolymerCount && !(haveProt && haveNucl);
            i++
          ) {
            if (this.bioPolymers[i].isNucleic()) haveNucl = true;
            else if (Clazz_instanceOf(this.bioPolymers[i], JM.AminoPolymer))
              haveProt = true;
          }
          var s = '';
          if (haveProt)
            s += J.api.Interface.getOption(
              'dssx.DSSP',
              this.vwr,
              'ms',
            ).calculateDssp(
          ...
          if (
            ...
            s += this.vwr
              .getAnnotationParser(true)
              .getHBonds(this.ms, this.modelIndex, vHBonds, doReport);
          return s;
        },
        'JU.Lst,~B,~B,~B,~N',
      );
        ...
        function (
          ...
          if (setStructure)
            for (var i = this.bioPolymerCount; --i >= 0; )
              if (
                !asDSSP ||
                this.bioPolymers[i].monomers[0].getNitrogenAtom() != null
              )
          ...
          if (!asDSSP || includeAlpha)
            for (var i = this.bioPolymerCount; --i >= 0; )
              if (Clazz_instanceOf(this.bioPolymers[i], JM.AlphaPolymer))
                this.bioPolymers[i].calculateStructures(includeAlpha);
      ...
      Clazz_defineMethod(c$, 'clearBioPolymers', function () {
      ...
      Clazz_overrideMethod(c$, 'freeze', function () {
        this.freezeM();
      ...
      Clazz_defineMethod(c$, 'getBioPolymerCount', function () {
      ...
      Clazz_defineMethod(
        ...
        function (key, ann) {
          ...
          if (annotv == null && ann != null) {
            annotv =
              Clazz_instanceOf(ann, JS.SV) ||
              Clazz_instanceOf(ann, java.util.Hashtable)
                ? ann
                : this.vwr.parseJSONMap(ann);
            cache.put(key, annotv);
          }
          return Clazz_instanceOf(annotv, JS.SV) ||
            Clazz_instanceOf(annotv, java.util.Hashtable)
            ? annotv
            : null;
        },
        '~S,~O',
            ...
            if (nAltLocs > 0) {
              ...
              for (
                ...
                if (
                  conformationIndex >= 0 &&
                  altloc != ch &&
                  !bsFound.get(altloc.charCodeAt(0))
                ) {
                  ch = altloc;
                  conformationIndex--;
        ...
        function (
          ...
          if (asDSSX && this.bioPolymerCount > 0) {
            ...
            for (var i = this.bioPolymerCount; --i >= 0; ) {
              ...
              for (var j = this.bioPolymerCount; --j >= 0; ) {
                if (
                  (bp1 = this.bioPolymers[j]) != null &&
                  (isRNA || i != j) &&
                  type == bp1.getType()
                ) {
                  bp1.calcRasmolHydrogenBonds(
                    bp,
                    bsA,
                    bsB,
                    vHBonds,
      ...
      Clazz_defineMethod(
        ...
        function (atom, flags) {
          ...
          sb.append('|')
            ...
            .append('|')
            .append(this.vwr.getChainIDStr(m.chain.chainID))
            .append('|')
            .append(m.getGroup3())
            .append('|')
            .appendI(m.getResno());
      ...
      Clazz_defineMethod(
        ...
        function (bs, dsspVersion) {
          ...
          for (var i = this.ms.bondCount; --i >= 0; ) {
            ...
            if (
              (bond.order & 28672) != 0 &&
              am[bond.atom1.mi].trajectoryBaseIndex == this.modelIndex
            )
              bsDelete.set(i);
          }
          if (bsDelete.nextSetBit(0) >= 0) this.ms.deleteBonds(bsDelete, false);
          this.getRasmolHydrogenBonds(
            bs,
            bs,
            null,
            false,
      ...
      Clazz_defineMethod(
        ...
        function (dssrData, dataType) {
          if (this.auxiliaryInfo.containsKey('dssr'))
            this.vwr
              .getAnnotationParser(true)
              .getAtomicDSSRData(this.ms, this.modelIndex, dssrData, dataType);
        },
        '~A,~S',
      );
    },
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (modelLoader) {
          ...
          if (modelLoader == null) {
            ...
            modelLoader.specialAtomIndexes = Clazz_newIntArray(
              JM.BioResolver.ATOMID_MAX,
              0,
            );
            this.hasCONECT =
      ...
      Clazz_defineMethod(
        ...
        function (vwr) {
          ...
          if (JM.Group.standardGroupList == null) {
            ...
            for (var i = 1; i < 42; i++)
              s.append(',[')
                .append(JM.BioResolver.predefinedGroup3Names[i])
                .append(']');
            ...
            for (
              var i = 0, n = JM.BioResolver.predefinedGroup3Names.length;
              i < n;
              ++i
            )
              JM.BioResolver.addGroup3Name(
                JM.BioResolver.predefinedGroup3Names[i].trim(),
              );
          }
          return this;
        },
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          var mask = 0;
          for (var i = JM.BioResolver.ATOMID_MAX; --i >= 0; )
            specialAtomIndexes[i] = -2147483648;
          ...
          var m = null;
          if ((mask & 14) == 14)
            m = JM.AminoMonomer.validateAndAllocate(
              chain,
              group3,
              seqcode,
              firstAtomIndex,
            ...
            m = JM.AlphaMonomer.validateAndAllocateA(
              chain,
              group3,
              seqcode,
              firstAtomIndex,
          ...
          else if ((mask & 8128) == 8128)
            m = JM.NucleicMonomer.validateAndAllocate(
              chain,
              group3,
              seqcode,
              firstAtomIndex,
            ...
            m = JM.PhosphorusMonomer.validateAndAllocateP(
              chain,
              group3,
              seqcode,
              firstAtomIndex,
          ...
          else if (JM.BioResolver.checkCarbohydrate(group3))
            m = JM.CarbohydrateMonomer.validateAndAllocate(
              chain,
              group3,
              seqcode,
              firstAtomIndex,
      ...
      Clazz_defineMethod(c$, 'initializeHydrogenAddition', function () {
      ...
      Clazz_defineMethod(
        ...
        function (adapter, iGroup, nH) {
          ...
          if (
            ...
            group3 == null ||
            (nH1 = JM.BioResolver.getStandardPdbHydrogenCount(group3)) == 0
          )
            return;
          nH = nH1 < 0 ? -1 : nH1 + nH;
          var model = null;
      ...
      Clazz_defineMethod(
        ...
        function (adapter, group3, model) {
          ...
          var bondInfo =
            model == null
              ? this.getPdbBondInfo(group3, this.vwr.getBoolean(603979873))
              : this.getLigandBondInfo(adapter, model, group3);
          ...
          for (var i = 0; i < bondInfo.length; i++) {
            ...
            if (bondInfo[i][1].charAt(0) == 'H')
              ...
              this.htBondMap.put(
                group3 + ':' + bondInfo[i][0] + ':' + bondInfo[i][1],
                bondInfo[i][2],
              );
          }
        },
              ...
              case 2:
                ...
                var d =
                  JU.Measure.distanceToPlane(
                    this.plane,
                    htAtoms.get(bondInfo[pt][1]),
                  ) * factor;
                bondInfo[pt][1] =
                  d > 0
                    ? bondInfo[pt][1] + '@' + bondInfo[pt + 1][1]
      ...
      Clazz_defineMethod(c$, 'finalizeHydrogens', function () {
        this.vwr.getLigandModel(null, null, null, null);
        this.finalizePdbMultipleBonds();
        this.addHydrogens();
      });
      Clazz_defineMethod(c$, 'addHydrogens', function () {
        ...
        var pts = this.ms.calculateHydrogens(
          this.bsAtomsForHs,
          nTotal,
          null,
          256,
      ...
      Clazz_defineMethod(c$, 'deleteUnneededAtoms', function () {
        var bsBondsDeleted = new JU.BS();
        for (
          var i = this.bsAtomsForHs.nextSetBit(0);
          i >= 0;
      ...
      Clazz_defineMethod(
        ...
        function (bsDeletedAtoms) {
          ...
          var mapNewToOld = Clazz_newIntArray(
            this.ms.ac - bsDeletedAtoms.cardinality(),
            0,
          );
          ...
          this.bsAddedMask = JU.BSUtil.deleteBits(
            this.bsAddedMask,
            bsDeletedAtoms,
          );
      ...
      Clazz_defineMethod(
        ...
        function (i, name, type) {
          ...
          if (o != null) {
            ...
            if (dbObj != null)
              this.vwr
                .getAnnotationParser(false)
                .fixAtoms(i, dbObj, this.bsAddedMask, type, 20);
          }
        },
        '~N,~S,~N',
      );
      Clazz_defineMethod(c$, 'finalizePdbCharges', function () {
        var atoms = this.ms.at;
        for (
          var i = this.bsAtomsForHs.nextSetBit(0);
          i >= 0;
      ...
      Clazz_defineMethod(c$, 'finalizePdbMultipleBonds', function () {
        ...
        for (
          var key, $key = this.htBondMap.keySet().iterator();
        ...
        for (var i = 0; i < bondCount; i++) {
          ...
          if (
            (value = htKeysBad.get(
              a1.getGroup3(false) + ':' + a1.getAtomName(),
            )) == null &&
            (value = htKeysBad.get(
              a2.getGroup3(false) + ':' + a2.getAtomName(),
            )) == null
          )
            continue;
          bonds[i].setOrder(JU.PT.parseInt(value));
          JU.Logger.info(
            'assigning order ' + bonds[i].order + ' to bond ' + bonds[i],
          );
        }
      });
      Clazz_defineMethod(
        ...
        function (iTo, iAtom, name, pt) {
          ...
          if (
            this.lastSetH == -2147483648 ||
            atoms[iAtom].mi != atoms[this.lastSetH].mi
          )
            this.maxSerial = this.ms.getInfo(
              atoms[(this.lastSetH = iAtom)].mi,
              'PDB_CONECT_firstAtom_count_max',
            )[2];
      ...
      Clazz_defineMethod(
        ...
        function (bsAtoms, data, toHydrogens) {
          ...
          var lastData = 0;
          for (
            var pt = 0, iAtom = 0, i = bsAtoms.nextSetBit(0);
            i >= 0;
            i = bsAtoms.nextSetBit(i + 1), iAtom++
          ) {
      ...
      c$.allocateBioPolymer = Clazz_defineMethod(
        c$,
        'allocateBioPolymer',
        function (groups, firstGroupIndex, checkConnections, pt0) {
          ...
          for (var i = firstGroupIndex; i < groups.length; ++i) {
            ...
            var current;
            if (
              !Clazz_instanceOf(group, JM.Monomer) ||
              (current = group).bioPolymer != null ||
              (previous != null &&
          ...
          for (var j = 0; j < count; ++j)
            monomers[j] = groups[firstGroupIndex + j];

          if (Clazz_instanceOf(previous, JM.AminoMonomer))
            return new JM.AminoPolymer(monomers, pt0);
          if (Clazz_instanceOf(previous, JM.AlphaMonomer))
            return new JM.AlphaPolymer(monomers, pt0);
          if (Clazz_instanceOf(previous, JM.NucleicMonomer))
            return new JM.NucleicPolymer(monomers);
          if (Clazz_instanceOf(previous, JM.PhosphorusMonomer))
            return new JM.PhosphorusPolymer(monomers);
          if (Clazz_instanceOf(previous, JM.CarbohydrateMonomer))
            return new JM.CarbohydratePolymer(monomers);
          JU.Logger.error(
      ...
      Clazz_defineMethod(
        ...
        function (adapter, atomSetCollection) {
          ...
          while (iterStructure.hasNext())
            if (iterStructure.getStructureType() !== J.c.STR.TURN)
      ...
      Clazz_defineMethod(
        ...
        function (iterStructure) {
          ...
          var m;
          if (bsAll != null) {
            for (var i = m0, t0; i <= m1; i++)
              if (Clazz_instanceOf((m = models[i]), JM.BioModel))
                for (var j = 0; j < 5; j++)
                  if (
                    (bs = bsAll[(t0 = JM.BioResolver.mytypes[j])]) != null &&
                    !bs.isEmpty()
                  )
                    m.addStructureByBS(0, t0, JM.BioResolver.types[j], bs);
          ...
          var startSeqCode = JM.Group.getSeqcodeFor(
            startSequenceNumber,
            startInsertionCode,
          );
          var endSeqCode = JM.Group.getSeqcodeFor(
            endSequenceNumber,
            endInsertionCode,
          );
          if (this.bsAssigned == null) this.bsAssigned = new JU.BS();
          for (var i = m0, i0 = 0; i <= m1; i++)
            if (Clazz_instanceOf((m = models[i]), JM.BioModel))
              m.addSecondaryStructure(
                type,
                id,
                serID,
                count,
                ...
                endChainID,
                endSeqCode,
                (i0 = m.firstAtomIndex) + atomRange[0],
                i0 + atomRange[1],
                this.bsAssigned,
              );
        },
      ...
      Clazz_defineMethod(
        ...
        function (ipt) {
          ...
          this.ml.group3Counts[ipt + 1] = Clazz_newIntArray(
            JM.BioResolver.group3Count + 10,
            0,
          );
          if (this.ml.group3Lists[0] == null) {
            this.ml.group3Lists[0] = JM.Group.standardGroupList;
            this.ml.group3Counts[0] = Clazz_newIntArray(
              JM.BioResolver.group3Count + 10,
              0,
            );
          }
      ...
      Clazz_defineMethod(
        ...
        function (g3, max) {
          ...
          return pt > 0
            ? pt < max
            : max == 2147483647 && JM.BioResolver.checkCarbohydrate(g3);
        },
        '~S,~N',
      );
      Clazz_defineMethod(
          ...
          if (JM.BioResolver.htSpecialAtoms == null) {
            ...
            for (var i = JM.BioResolver.specialAtomNames.length; --i >= 0; ) {
              ...
              if (specialAtomName != null)
                JM.BioResolver.htSpecialAtoms.put(
                  specialAtomName,
                  Byte.$valueOf(i),
                );
            }
      ...
      Clazz_defineMethod(
        ...
        function (group3, isLegacy) {
          if (JM.BioResolver.htPdbBondInfo == null)
      ...
      c$.knownGroupID = Clazz_defineMethod(
        c$,
        'knownGroupID',
        function (group3) {
      ...
      c$.checkCarbohydrate = Clazz_defineMethod(
        c$,
        'checkCarbohydrate',
        function (group3) {
          return (
      ...
      Clazz_defineMethod(
        ...
        function (g1) {
          ...
          if (pt < 0) {
            for (var i = 1; i <= 20; i++) {
              pt = JM.BioResolver.knownGroupID(
                JM.BioResolver.predefinedGroup3Names[i],
              );
              JM.BioResolver.htGroup.put(
                '==' + JM.BioResolver.predefinedGroup1Names[i],
                Short.$valueOf(pt),
              );
            }
          }
      ...
      c$.getGroupIdFor = Clazz_defineMethod(
        c$,
        'getGroupIdFor',
        function (group3) {
      ...
      c$.addGroup3Name = Clazz_defineMethod(
        c$,
        'addGroup3Name',
        function (group3) {
          if (JM.BioResolver.group3NameCount == JM.Group.group3Names.length)
      ...
      c$.getStandardPdbHydrogenCount = Clazz_defineMethod(
        c$,
        'getStandardPdbHydrogenCount',
        function (group3) {
          var pt = JM.BioResolver.knownGroupID(group3);
          return pt < 0 || pt >= JM.BioResolver.pdbHydrogenCount.length
            ? -1
            : JM.BioResolver.pdbHydrogenCount[pt];
        },
        '~S',
      );
      c$.getSpecialAtomName = Clazz_defineMethod(
        c$,
        'getSpecialAtomName',
        function (atomID) {
      ...
      Clazz_defineMethod(
        ...
        function (modelSet) {
          if (modelSet.bioModelset == null)
            modelSet.bioModelset = new JM.BioModelSet().set(this.vwr, modelSet);
          return modelSet.bioModelset;
        },
        'JM.ModelSet',
      );
      c$.htGroup = c$.prototype.htGroup = new java.util.Hashtable();
      c$.types = c$.prototype.types = Clazz_newArray(-1, [
        J.c.STR.HELIXPI,
        J.c.STR.HELIXALPHA,
        J.c.STR.SHEET,
        J.c.STR.HELIX310,
      ...
      Clazz_defineStatics(
        ...
        Clazz_newArray(-1, [
          '',
          'N N CA HA C O CB HB?',
          'N N CA HA C O CB B CG G CD D NE HE CZ NH1 NH1 HH11@HH12 NH2 HH22@HH21',
          'N N CA HA C O CB B CG OD1 ND2 HD21@HD22',
          'N N CA HA C O CB B CG OD1',
          'N N CA HA C O CB B SG HG',
          'N N CA HA C O CB B CG G CD OE1 NE2 HE22@HE21',
          'N N CA HA C O CB B CG G CD OE1',
          'N N CA HA2@HA3 C O',
          'N N CA HA C O CB B CG CD2 ND1 CE1 ND1 HD1 CD2 HD2 CE1 HE1 NE2 HE2',
          'N N CA HA C O CB HB CG1 HG13@HG12 CG2 HG2? CD1 HD1?',
          'N N CA HA C O CB B CG HG CD1 HD1? CD2 HD2?',
          'N N CA HA C O CB B CG G CD HD2@HD3 CE HE3@HE2 NZ HZ?',
          'N N CA HA C O CB B CG G CE HE?',
          'N N CA HA C O CB B CG CD1 CD1 HD1 CD2 CE2 CD2 HD2 CE1 CZ CE1 HE1 CE2 HE2 CZ HZ',
          'N H CA HA C O CB B CG G CD D',
          'N N CA HA C O CB B OG HG',
          'N N CA HA C O CB HB OG1 HG1 CG2 HG2?',
          'N N CA HA C O CB B CG CD1 CD1 HD1 CD2 CE2 NE1 HE1 CE3 CZ3 CE3 HE3 CZ2 CH2 CZ2 HZ2 CZ3 HZ3 CH2 HH2',
          'N N CA HA C O CB B CG CD1 CD1 HD1 CD2 CE2 CD2 HD2 CE1 CZ CE1 HE1 CE2 HE2 OH HH',
          'N N CA HA C O CB HB CG1 HG1? CG2 HG2?',
          'N N CA HA C O CB B',
          'N N CA HA C O CB B CG G',
          '',
          "P OP1 C5' 5 C4' H4' C3' H3' C2' H2' O2' HO2' C1' H1' C8 N7 C8 H8 C5 C4 C6 O6 N1 H1 C2 N3 N2 H22@H21 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' H2' O2' HO2' C1' H1' C2 O2 N3 C4 N4 H41@H42 C5 C6 C5 H5 C6 H6 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' H2' O2' HO2' C1' H1' C8 N7 C8 H8 C5 C4 C6 N1 N6 H61@H62 C2 N3 C2 H2 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C2 O2 N3 H3 C4 O4 C5 C6 C7 H7? C6 H6 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' H2' O2' HO2' C1' H1' C2 O2 N3 H3 C4 O4 C5 C6 C5 H5 C6 H6 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' H2' O2' HO2' C1' H1' C8 N7 C8 H8 C5 C4 C6 O6 N1 H1 C2 N3 C2 H2 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C8 N7 C8 H8 C5 C4 C6 O6 N1 H1 C2 N3 N2 H22@H21 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C2 O2 N3 C4 N4 H41@H42 C5 C6 C5 H5 C6 H6 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C8 N7 C8 H8 C5 C4 C6 N1 N6 H61@H62 C2 N3 C2 H2 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C2 O2 N3 H3 C4 O4 C5 C6 C7 H7? C6 H6 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C2 O2 N3 H3 C4 O4 C5 C6 C5 H5 C6 H6 O3' HO3' O5' HO5'",
          "P OP1 C5' 5 C4' H4' C3' H3' C2' 2 C1' H1' C8 N7 C8 H8 C5 C4 C6 O6 N1 H1 C2 N3 C2 H2 O3' HO3' O5' HO5'",
        ]),
        'pdbHydrogenCount',
        Clazz_newIntArray(
          -1,
      ...
      Clazz_defineStatics(
        ...
        Clazz_newIntArray(
          -1,
          [
            0xffbea06e, 0xffc8c8c8, 0xff145aff, 0xff00dcdc, 0xffe60a0a,
            0xffe6e600, 0xff00dcdc, 0xffe60a0a, 0xffebebeb, 0xff8282d2,
            0xff0f820f, 0xff0f820f, 0xff145aff, 0xffe6e600, 0xff3232aa,
            0xffdc9682, 0xfffa9600, 0xfffa9600, 0xffb45ab4, 0xff3232aa,
            0xff0f820f, 0xffff69b4, 0xffff69b4, 0xffbea06e,
          ],
        ),
        'argbsNucleic',
        Clazz_newIntArray(
          -1,
          [
            0xffbea06e, 0xffa0a0a0, 0xff0f820f, 0xffe6e600, 0xffe60a0a,
            0xff145aff, 0xff00dcdc, 0xff00dcdc, 0xff0f820f, 0xffe6e600,
            0xffe60a0a, 0xff145aff, 0xff00dcdc, 0xff00dcdc, 0xff0f820f,
            0xffe6e600, 0xffe60a0a, 0xff145aff, 0xff00dcdc, 0xff00dcdc,
          ],
        ),
        'argbsChainAtom',
        Clazz_newIntArray(
          -1,
          [
            0xffffffff, 0xffc0d0ff, 0xffb0ffb0, 0xffffc0c8, 0xffffff80,
            0xffffc0ff, 0xffb0f0f0, 0xffffd070, 0xfff08080, 0xfff5deb3,
            0xff00bfff, 0xffcd5c5c, 0xff66cdaa, 0xff9acd32, 0xffee82ee,
            0xff00ced1, 0xff00ff7f, 0xff3cb371, 0xff00008b, 0xffbdb76b,
            0xff006400, 0xff800000, 0xff808000, 0xff800080, 0xff008080,
            0xffb8860b, 0xffb22222,
          ],
        ),
        'argbsChainHetero',
        Clazz_newIntArray(
          -1,
          [
            0xffffffff, -7298865, -8335464, -3174224, -3158160, -3174193,
        ...
        Clazz_newIntArray(
          -1,
          [
            0xffff00ff, 0xff00007c, 0xffff7c70, 0xff8cff8c, 0xffa00042,
            0xffffff70, 0xffff4c4c, 0xff660000, 0xffffffff, 0xff7070ff,
            0xff004c00, 0xff455e45, 0xff4747b8, 0xff534c52, 0xffb8a042,
            0xff525252, 0xffff7042, 0xffb84c00, 0xff4f4600, 0xff8c704c,
            0xffff8cff, 0xffff00ff, 0xffff00ff, 0xffff00ff, 0xffff7070,
            0xffff8c4b, 0xffa0a0ff, 0xffa0ffa0, 0xffff8080, 0xff80ffff,
            0xffff7070, 0xffff8c4b, 0xffa0a0ff, 0xffa0ffa0, 0xffff8080,
            0xff80ffff, 0xffff7070, 0xffff8c4b, 0xffa0a0ff, 0xffa0ffa0,
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'isCarbohydrate', function () {
        return true;
      });
      Clazz_overrideMethod(c$, 'getProteinStructureType', function () {
      ...
      Clazz_overrideMethod(
        ...
        function (possiblyPreviousMonomer) {
          ...
          for (var i = this.firstAtomIndex; i <= this.lastAtomIndex; i++)
            for (
              var j = possiblyPreviousMonomer.firstAtomIndex;
              j <= possiblyPreviousMonomer.lastAtomIndex;
              j++
            ) {
              ...
              if (
                a != null &&
                b != null &&
                a.getElementNumber() + b.getElementNumber() == 14 &&
                a.distanceSquared(b) < 3.24
              )
                return true;
            }
      ...
      Clazz_overrideMethod(
        ...
        function (x, y, closest, madBegin, madEnd) {
          ...
          var radiusBegin = Clazz_floatToInt(
            this.scaleToScreen(anomericO.sZ, marBegin),
          );
          if (radiusBegin < 4) radiusBegin = 4;
      ...
      Clazz_overrideMethod(c$, 'isConnectedPrevious', function () {
        if (this.monomerIndex <= 0) return false;
        for (var i = this.firstAtomIndex; i <= this.lastAtomIndex; i++)
          if (this.getCrossLinkGroup(i, null, null, true, false, false))
            return true;
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'isNucleic', function () {
        return true;
      });
      Clazz_overrideConstructor(c$, function () {});
      c$.validateAndAllocateP = Clazz_defineMethod(
        c$,
        ...
        function (
          ...
          return firstIndex != lastIndex || specialAtomIndexes[13] != firstIndex
            ? null
      ...
      Clazz_overrideMethod(c$, 'isDna', function () {
        return this.isDnaByID();
      });
      Clazz_overrideMethod(c$, 'isRna', function () {
        return this.isRnaByID();
      });
      Clazz_overrideMethod(c$, 'isPurine', function () {
        return this.isPurineByID();
      });
      Clazz_overrideMethod(c$, 'isPyrimidine', function () {
      ...
      Clazz_defineMethod(
        ...
        function (possiblyPreviousMonomer) {
          if (possiblyPreviousMonomer == null) return true;
          var distance = this.getLeadAtom().distance(
            possiblyPreviousMonomer.getLeadAtom(),
          );
          return (
            distance <= JM.PhosphorusMonomer.MAX_ADJACENT_PHOSPHORUS_DISTANCE
          );
        },
        'JM.Monomer',
      );
      ...
      Clazz_defineMethod(c$, 'getQuaternionP', function () {
    ...
    function () {
      ...
      c$.validateAndAllocate = Clazz_defineMethod(
        ...
        function (
          ...
          JM.Monomer.checkOptional(
            ...
            20,
            firstAtomIndex,
            specialAtomIndexes[89],
          );
          JM.Monomer.checkOptional(
            offsets,
            18,
            firstAtomIndex,
            specialAtomIndexes[90],
          );
          JM.Monomer.checkOptional(
            offsets,
            23,
            firstAtomIndex,
            specialAtomIndexes[75],
          );
          JM.Monomer.checkOptional(
            offsets,
            24,
            firstAtomIndex,
            specialAtomIndexes[77],
          );
          return new JM.NucleicMonomer().set4(
            chain,
            group3,
            seqcode,
            firstAtomIndex,
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          this.$isPurine =
            JM.Monomer.have(offsets, 9) &&
            JM.Monomer.have(offsets, 10) &&
            JM.Monomer.have(offsets, 11);
          return this;
        },
        'JM.Chain,~S,~N,~N,~N,~A',
      );
      Clazz_overrideMethod(c$, 'isNucleicMonomer', function () {
        return true;
      });
      Clazz_overrideMethod(c$, 'isDna', function () {
      ...
      Clazz_defineMethod(c$, 'isGuanine', function () {
        return JM.Monomer.have(this.offsets, 17);
      });
      Clazz_overrideMethod(c$, 'getProteinStructureType', function () {
        return this.hasRnaO2Prime ? J.c.STR.RNA : J.c.STR.DNA;
      });
      Clazz_defineMethod(c$, 'getP', function () {
        return this.getAtomFromOffsetIndex(0);
      });
      Clazz_defineMethod(c$, 'getC1P', function () {
        return this.getAtomFromOffsetIndex(25);
      });
      Clazz_defineMethod(c$, 'getC2', function () {
        return this.getAtomFromOffsetIndex(5);
      });
      Clazz_defineMethod(c$, 'getC5', function () {
        return this.getAtomFromOffsetIndex(3);
      });
      Clazz_defineMethod(c$, 'getC6', function () {
        return this.getAtomFromOffsetIndex(1);
      });
      Clazz_defineMethod(c$, 'getC8', function () {
        return this.getAtomFromOffsetIndex(10);
      });
      Clazz_defineMethod(c$, 'getC4P', function () {
        return this.getAtomFromOffsetIndex(27);
      });
      Clazz_defineMethod(c$, 'getN1', function () {
        return this.getAtomFromOffsetIndex(4);
      });
      Clazz_defineMethod(c$, 'getN3', function () {
        return this.getAtomFromOffsetIndex(6);
      });
      Clazz_defineMethod(c$, 'getN2', function () {
        return this.getAtomFromOffsetIndex(17);
      });
      Clazz_defineMethod(c$, 'getN4', function () {
        return this.getAtomFromOffsetIndex(14);
      });
      Clazz_defineMethod(c$, 'getN6', function () {
        return this.getAtomFromOffsetIndex(16);
      });
      Clazz_defineMethod(c$, 'getO2', function () {
        return this.getAtomFromOffsetIndex(8);
      });
      Clazz_defineMethod(c$, 'getO4', function () {
        return this.getAtomFromOffsetIndex(12);
      });
      Clazz_defineMethod(c$, 'getO6', function () {
      ...
      Clazz_defineMethod(
        ...
        function (pts) {
          if (this.$isPurine)
            this.getPoints(JM.NucleicMonomer.ring5OffsetIndexes, pts);
      ...
      Clazz_overrideMethod(
        ...
        function (possiblyPreviousMonomer) {
          ...
          return (
            possiblyPreviousMonomer
              .getAtomFromOffsetIndex(21)
              .isBonded(myPhosphorusAtom) || this.isCA2(possiblyPreviousMonomer)
          );
        },
        'JM.Monomer',
      );
      Clazz_overrideMethod(
        ...
        function (x, y, closest, madBegin, madEnd) {
          ...
          if (
            this.isCursorOnTopOf(lead, x, y, radius, competitor) ||
            this.isCursorOnTopOf(o5prime, x, y, radius, competitor) ||
            this.isCursorOnTopOf(c3prime, x, y, radius, competitor)
          )
            closest[0] = lead;
        },
        '~N,~N,~A,~N,~N',
        ...
        function (isVisible) {
          for (var i = 6; --i >= 0; )
            this.getAtomFromOffsetIndex(
              JM.NucleicMonomer.ring6OffsetIndexes[i],
            ).setShapeVisibility(32768, isVisible);

          if (this.$isPurine)
            for (var i = 4; --i >= 1; )
              this.getAtomFromOffsetIndex(
                JM.NucleicMonomer.ring5OffsetIndexes[i],
              ).setShapeVisibility(32768, isVisible);
        },
        '~B',
      );
      Clazz_defineMethod(c$, 'setRingsClickable', function () {
        for (var i = 6; --i >= 0; )
          this.getAtomFromOffsetIndex(
            JM.NucleicMonomer.ring6OffsetIndexes[i],
          ).setClickable(32768);
      ...
      Clazz_defineMethod(c$, 'getN0', function () {
          ...
          switch (qType) {
            ...
            case 'c':
              if (this.baseCenter == null) {
                ...
                for (
                  var i = 0;
                  i < JM.NucleicMonomer.heavyAtomIndexes.length;
                  i++
                ) {
                  var a = this.getAtomFromOffsetIndex(
                    JM.NucleicMonomer.heavyAtomIndexes[i],
                  );
                  if (a == null) continue;
      ...
      Clazz_overrideMethod(
        ...
        function (g) {
          if (
            !Clazz_instanceOf(g, JM.NucleicMonomer) ||
            this.$isPurine == g.isPurine()
          )
            return false;
      ...
      Clazz_defineMethod(c$, 'getBasePairs', function () {
        if (this.bioPolymer != null && !this.bioPolymer.isDssrSet)
      ...
      Clazz_overrideMethod(c$, 'getGroup1b', function () {
        var g3 = JM.Group.group3Names[this.groupID];
        var g1 =
          JM.NucleicPolymer.htGroup1 == null
            ? null
            : JM.NucleicPolymer.htGroup1.get(g3);
        return g1 == null
          ? Character.toLowerCase(g3.charAt(g3.length - 1))
          : g1.charAt(0);
      });
      Clazz_defineMethod(
        c$,
      ...
      Clazz_defineStatics(
        ...
        'ring6OffsetIndexes',
        Clazz_newByteArray(-1, [3, 1, 4, 5, 6, 7]),
        'ring5OffsetIndexes',
        Clazz_newByteArray(-1, [3, 9, 10, 11, 7]),
        'riboseOffsetIndexes',
        Clazz_newByteArray(-1, [25, 26, 22, 27, 28, 21, 29, 19, 0]),
        'heavyAtomIndexes',
        Clazz_newByteArray(
          -1,
          [3, 1, 4, 5, 6, 7, 11, 10, 9, 16, 14, 8, 12, 17, 13],
        ),
      );
    },
  );
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'calcEtaThetaAngles', function () {
        var eta = NaN;
        ...
        function (
          ...
          for (var i = this.monomerCount; --i >= 0; ) {
            ...
            for (var j = other.monomerCount; --j >= 0; ) {
              ...
              if (
                dist2 < minDist2 &&
                myN9.distanceSquared(otherN1) > 50 &&
                Math.abs(JU.Measure.distanceToPlane(plane, otherN3)) < 1
              ) {
            ...
            if (bestN3 != null) {
              ...
              if (myNucleotide.isGuanine()) {
                n += JM.NucleicPolymer.addHydrogenBond(
                  vAtoms,
                  myNucleotide.getN2(),
      ...
      c$.addHydrogenBond = Clazz_defineMethod(
        c$,
        'addHydrogenBond',
      ...
      Clazz_defineStatics(c$, 'htGroup1', null);
    },
  );
  Clazz_declarePackage('JM');
  c$ = Clazz_decorateAsClass(
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (bridge, htLadders) {
          if (
            bridge.isAntiparallel != this.isAntiparallel ||
            !this.canAdd(bridge) ||
            !bridge.canAdd(this)
          )
            return false;
      ...
      Clazz_defineMethod(
        ...
        function (htLadders) {
          htLadders.put(
            this.ladder,
            this.isAntiparallel ? Boolean.TRUE : Boolean.FALSE,
          );
        },
        'java.util.Map',
      );
      Clazz_defineMethod(
        ...
        function (bridge) {
          ...
          return this.isAntiparallel
            ? (index1 >= this.ladder[0][1] && index2 <= this.ladder[1][0]) ||
                (index1 <= this.ladder[0][0] && index2 >= this.ladder[1][1])
            : (index1 <= this.ladder[0][0] && index2 <= this.ladder[1][0]) ||
                (index1 >= this.ladder[0][1] && index2 >= this.ladder[1][1]);
        },
        'J.dssx.Bridge',
      );
      Clazz_defineMethod(
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_prepareFields(c$, function () {
        this.sheetOffsets = Clazz_newArray(-1, [
          Clazz_newIntArray(-1, [0, -1, 1, 0, 1, 0, 0, -1]),
          Clazz_newIntArray(-1, [0, 0, 0, 0, 1, -1, 1, -1]),
        ]);
      });
      Clazz_makeConstructor(c$, function () {});
      Clazz_defineMethod(
        ...
        function (
          ...
          for (var i = 0; i < bioPolymerCount; i++)
            if (Clazz_instanceOf(this.bioPolymers[i], JM.AminoPolymer))
              bsAmino.set(i);
          ...
          sb.append('Jmol ')
            ...
            .append(m.ms.getModelNumberDotted(m.modelIndex))
            .append(' - ')
            .append(m.ms.getModelTitle(m.modelIndex))
            .append('\n');
          if (m.modelIndex == 0)
            sb.append(
          ...
          for (
            ...
            for (var j = 0; j < n; j++)
              if (ap.monomers[j].getCarbonylOxygenAtom() == null)
          ...
          for (
            ...
            if (min[i] != null) reports[i] = this.findHelixes(i, min[i]);
            ...
            for (
              ...
              if (this.labels[i] != null) {
                ...
                sb.append(reports[i]).append(
                  this.dumpTags(
                    ap,
                    '$.1: ' + String.valueOf(this.labels[i]),
                    this.bsBad,
                    2,
                  ),
                );
      ...
      Clazz_defineMethod(c$, 'getDualHydrogenBondArray', function () {
        ...
        for (var i = 0; i < this.bioPolymerCount; i++)
          if (min[i] != null)
            for (var j = 0; j < this.bioPolymerCount; j++)
              if (min[j] != null)
                this.bioPolymers[i].calcRasmolHydrogenBonds(
                  this.bioPolymers[j],
                  null,
                  null,
                  null,
      ...
      Clazz_defineMethod(
        ...
        function (min) {
          ...
          for (var p1 = 0; p1 < min.length; p1++)
            if (Clazz_instanceOf(this.bioPolymers[p1], JM.AminoPolymer)) {
              var ap1 = this.bioPolymers[p1];
              var n = min[p1].length - 1;
              for (var a = 1; a < n; a++) {
                var ia = ap1.monomers[a].leadAtomIndex;
                if (this.bsBad.get(ia)) continue;
                for (var p2 = p1; p2 < min.length; p2++)
                  if (Clazz_instanceOf(this.bioPolymers[p2], JM.AminoPolymer))
                    for (
                      var b = p1 == p2 ? a + 3 : 1;
                      b < min[p2].length - 1;
                      b++
                      ...
                      if (JU.Logger.debugging)
                        JU.Logger.debug('Bridge found ' + bridge);
                      this.done[p1].set(a);
                      this.done[p2].set(b);
        ...
        function (
          ...
          if (
            ...
            if (this.vHBonds != null) {
              ...
              this.addHbond(
                ap1.monomers[a + offsets[ipt]],
                ap2.monomers[b + offsets[++ipt]],
                b1[2],
                type,
                htTemp,
              );
      ...
      Clazz_defineMethod(
        ...
        function (donor, acceptor, iEnergy, type, htTemp) {
          ...
          this.vHBonds.addLast(
            new JM.HBond(nitrogen, oxygen, type, 1, 0, iEnergy / 1000),
          );
        },
        'JM.Monomer,JM.Monomer,~N,~N,java.util.Map',
      );
      Clazz_defineMethod(c$, 'getSheetStructures', function () {
        ...
        var bsB = new JU.BS();
        for (
          var ladder, $ladder = this.htLadders.keySet().iterator();
          $ladder.hasNext() && ((ladder = $ladder.next()) || true);
      ...
      Clazz_defineMethod(
        ...
        function (bridges, isAntiparallel) {
          ...
          var n = bridges.size();
          for (var i = 0; i < n; i++)
            this.checkBridge(bridges.get(i), isAntiparallel, 1, dir);

          for (var i = 0; i < n; i++)
            this.checkBulge(bridges.get(i), isAntiparallel, 1);
        },
        'JU.Lst,~B',
      );
      Clazz_defineMethod(
        ...
        function (bridge, isAntiparallel, n1, n2) {
          var b = this.htBridges.get(
            bridge.a.getOffsetResidueAtom('\0', n1) +
              '-' +
      ...
      Clazz_defineMethod(
        ...
        function (ap, lines, bsBad, mode) {
          var prefix =
            ap.monomers[0].getLeadAtom().getChainID() +
            '.' +
          ...
          for (var ii = 0; ii < n; ii++) {
            ...
            sb0.append(
              i % 100 == 0 ? '' + (Clazz_doubleToInt(i / 100) % 100) : ' ',
            );
            sb1.append(
              i % 10 == 0 ? '' + (Clazz_doubleToInt(i / 10) % 10) : ' ',
            );
            sb2.appendI(i % 10);
            sb3.appendC(
              bsBad.get(ap.monomers[ii].leadAtomIndex)
                ? '!'
      ...
      Clazz_defineMethod(
        ...
        function (indexDonor, indexAcceptor, pDonor, pAcceptor, min) {
          ...
          return min1[indexDonor][0][0] == pAcceptor &&
            min1[indexDonor][0][1] == indexAcceptor
            ? min1[indexDonor][0]
            : min1[indexDonor][1][0] == pAcceptor &&
              min1[indexDonor][1][1] == indexAcceptor
            ? min1[indexDonor][1]
            : null;
        },
        '~N,~N,~N,~N,~A',
      ...
      Clazz_defineMethod(
        ...
        function (iPolymer, min) {
          ...
          if (JU.Logger.debugging)
            for (var j = 0; j < ap.monomerCount; j++)
              JU.Logger.debug(
                iPolymer +
                  '.' +
                  ap.monomers[j].getResno() +
                  '\t' +
                  JU.Escape.e(min[j]),
              );
          ...
          if (this.setStructure)
            ap.setStructureBS(0, 6, J.c.STR.TURN, bsTurn, false);
          if (this.doReport) {
      ...
      Clazz_defineMethod(
        ...
        function (
          ...
          for (var i = pitch; i < n; ++i) {
            ...
            var bpt = 0;
            if (
              (min[i][0][0] == iPolymer && min[i][0][1] == i0) ||
              ...
              if (i0 > 0 && bsStart.get(i0 - 1) && (isFirst || isClear)) {
                ...
                if (!isClear)
                  warning +=
                    '  WARNING! Bridge to helix at ' + ap.monomers[ipt];
                addH = true;
          ...
          if (this.doReport) {
            this.setTag(
              this.labels[iPolymer],
              bsHelix,
              String.fromCharCode(68 + pitch),
            );
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'calcBfactorRange', function () {
      ...
      Clazz_defineMethod(
        ...
        function (bFactor100) {
          return Clazz_doubleToShort(
            Math.sqrt(bFactor100 / 7895.6835208714865) * 1000,
          );
        },
        '~N',
      );
      ...
      Clazz_defineMethod(
        ...
        function (mad, bsSelected, values) {
          ...
          var setRingVis =
            flag == 32768 &&
            Clazz_instanceOf(this.bioPolymer, JM.NucleicPolymer);
          for (var i = this.monomerCount; --i >= 0; ) {
            ...
            if (bsSelected.get(leadAtomIndex)) {
              ...
              this.shape.atoms[leadAtomIndex].setShapeVisibility(
                flag,
                isVisible,
              );
          ...
          switch (mad) {
            ...
            case -3: {
              ...
              if (percentile < 0 || percentile > 1)
                JU.Logger.error('Que ha ocurrido? ' + percentile);
              return Clazz_floatToShort(1750 * percentile + 250);
            }
            case -4: {
              var atom = this.monomers[groupIndex].getLeadAtom();
              return (
                2 * this.calcMeanPositionalDisplacement(atom.getBfactor100())
              );
            }
          }
      ...
      Clazz_defineMethod(c$, 'falsifyMesh', function () {
      ...
      Clazz_defineMethod(
        ...
        function (colix, bsSelected) {
          if (this.colixesBack == null)
            this.colixesBack = Clazz_newShortArray(this.colixes.length, 0);
          ...
          for (var i = this.monomerCount; --i >= 0; )
            if (bsSelected.get(this.leadAtomIndices[i]))
        ...
        function (atomColixes, bsSelected) {
          ...
          for (var i = this.monomerCount; --i >= 0; ) {
            ...
            if (
              ...
              i < this.colixes.length &&
              atomIndex < atomColixes.length
            ) {
              this.colixes[i] = this.shape.getColixI(
                atomColixes[atomIndex],
                J.c.PAL.UNKNOWN.id,
                atomIndex,
              );
      ...
      Clazz_defineMethod(
        ...
        function (isTranslucent, bsSelected, translucentLevel) {
          ...
          for (var i = this.monomerCount; --i >= 0; )
            if (bsSelected.get(this.leadAtomIndices[i])) {
              this.colixes[i] = JU.C.getColixTranslucent3(
                this.colixes[i],
                isTranslucent,
                translucentLevel,
              );
              if (this.colixesBack != null && this.colixesBack.length > i)
                this.colixesBack[i] = JU.C.getColixTranslucent3(
                  this.colixesBack[i],
                  isTranslucent,
                  translucentLevel,
                );
      ...
      Clazz_overrideMethod(c$, 'setAtomClickability', function () {
        if (
          !this.isActive ||
          this.wingVectors == null ||
          this.monomerCount == 0
          ...
          return;
        var setRingsClickable =
          Clazz_instanceOf(this.bioPolymer, JM.NucleicPolymer) &&
          this.shape.shapeID == 11;
        var setAlphaClickable =
          Clazz_instanceOf(this.bioPolymer, JM.AlphaPolymer) ||
          this.shape.shapeID != 15;
      ...
      Clazz_defineMethod(
        ...
        function (type, translucentAllowed, temp, temp2) {
          if (this.monomerCount > 0) {
            if (
              !this.isActive ||
              (this.bsSizeSet == null && this.bsColixSet == null)
            )
              return;
            for (var i = 0; i < this.monomerCount; i++) {
              ...
              if (
                ...
                if (this.bsSizeDefault.get(i)) {
                  JU.BSUtil.setMapBitSet(
                    ...
                    atomIndex1,
                    atomIndex2,
                    type + (this.bsSizeSet.get(i) ? ' on' : ' off'),
                  );
                } else {
                  JU.BSUtil.setMapBitSet(
                    temp,
                    atomIndex1,
                    atomIndex2,
                    type + ' ' + JU.PT.escF(this.mads[i] / 2000),
                  );
                }
              }
              if (this.bsColixSet == null || !this.bsColixSet.get(i)) continue;
              var s = J.shape.Shape.getColorCommand(
                type,
                this.paletteIDs[i],
                this.colixes[i],
                translucentAllowed,
              ...
              if (
                this.colixesBack != null &&
                this.colixesBack.length > i &&
                this.colixesBack[i] != 0
              )
                s += ' ' + JU.C.getHexCode(this.colixesBack[i]);
      ...
      Clazz_defineStatics(c$, 'eightPiSquared100', 7895.6835208714865);
    },
  );
  Clazz_declarePackage('J.shapebio');
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'initModelSet', function () {
        this.isBioShape = true;
        this.atoms = this.ms.at;
        this.initialize();
      });
      Clazz_overrideMethod(c$, 'initShape', function () {});
      Clazz_overrideMethod(
        c$,
        'getSizeG',
        function (group) {
          ...
          for (var i = this.bioShapes.length; --i >= 0; ) {
            ...
            for (var j = 0; j < bioShape.monomerCount; j++) {
              if (
                bioShape.monomers[j].groupIndex == groupIndex &&
      ...
      Clazz_defineMethod(
        ...
        function (propertyName, value, bsSelected) {
          ...
          if ('params' === propertyName) {
            ...
            for (
              var pt = 0, i = bsSelected.nextSetBit(0);
              i >= 0;
              i = bsSelected.nextSetBit(i + 1), pt++
            )
              atomMap[i] = pt;

            for (var i = this.bioShapes.length; --i >= 0; )
              this.bioShapes[i].setParams(value, atomMap, bsSelected);
          ...
          if ('translucency' === propertyName) {
            ...
            for (var i = this.bioShapes.length; --i >= 0; ) {
              var bioShape = this.bioShapes[i];
              if (bioShape.monomerCount > 0)
                bioShape.setTranslucent(
                  isTranslucent,
                  bsSelected,
                  this.translucentLevel,
                );
            }
            return;
          }
      ...
      Clazz_overrideMethod(c$, 'getShapeState', function () {
        ...
        for (var iShape = this.bioShapes.length; --iShape >= 0; )
          this.bioShapes[iShape].getBioShapeState(
            type,
        ...
        var s =
          '\n' +
          this.vwr.getCommands(
            temp,
            temp2,
            this.shapeID == 9 ? 'Backbone' : 'select',
          );
        return s;
      });
      Clazz_defineMethod(c$, 'initialize', function () {
        ...
        for (var i = modelCount; --i >= 0; )
          for (var j = this.ms.getBioPolymerCountInModel(i); --j >= 0; n--) {
            var bp = models[i].bioPolymers[j];
            shapes[n] =
              this.bioShapes == null ||
              this.bioShapes.length <= n ||
              this.bioShapes[n] == null ||
              this.bioShapes[n].bioPolymer !== bp
                ? new J.shapebio.BioShape(this, i, bp)
                : this.bioShapes[n];
          }
      ...
      Clazz_overrideMethod(
        ...
        function (xMouse, yMouse, closest, bsNot) {
          for (var i = this.bioShapes.length; --i >= 0; )
            this.bioShapes[i].findNearestAtomIndex(
              xMouse,
              yMouse,
              closest,
              bsNot,
      ...
      Clazz_defineMethod(c$, 'getMpsShapeCount', function () {
      ...
      Clazz_overrideMethod(
        ...
        function (size, rd, bsSelected) {
          ...
          for (var iShape = this.bioShapes.length; --iShape >= 0; ) {
            ...
            for (
              var i = n - (bioShape.bioPolymer.isCyclic() ? 0 : 1);
              --i >= 0;
              ...
              if (
                ...
                if (rd != null) {
                  if (
                    Float.isNaN(rd.values[index1]) ||
                    Float.isNaN(rd.values[index2])
                  )
                    continue;
                  mad = Clazz_floatToShort(
                    (rd.values[index1] + rd.values[index2]) * 1000,
                  );
                  isVisible = mad != 0;
                }
      ...
      Clazz_overrideMethod(c$, 'setAtomClickability', function () {
        ...
        for (var iShape = this.bioShapes.length; --iShape >= 0; ) {
          ...
          for (var i = bioShape.monomerCount; --i >= 0; ) {
            ...
            if (atom.nBackbonesDisplayed > 0 && !this.ms.isAtomHidden(atom.i))
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (info, bsAtoms) {
          ...
          var rd = new J.atomdata.RadiusData(
            data,
            0,
  ...
  Clazz_load(
    ...
    [
      'javajs.api.Interface',
      'J.c.STR',
      'JM.CarbohydratePolymer',
      '$.NucleicPolymer',
      '$.PhosphorusPolymer',
      'JU.C',
    ],
    function () {
      ...
      Clazz_defineMethod(c$, 'setGlobals', function () {
        ...
        if (val != this.aspectRatio && val != 0 && val1 != 0)
          this.invalidateMesh = true;
      ...
      Clazz_defineMethod(c$, 'renderShapes', function () {
        var mps = this.shape;
        for (var c = mps.bioShapes.length; --c >= 0; ) {
          this.bioShape = mps.getBioShape(c);
          if ((this.bioShape.modelVisibilityFlags & this.myVisibilityFlag) == 0)
            continue;
          if (
            this.bioShape.monomerCount >= 2 &&
      ...
      Clazz_defineMethod(c$, 'freeTempArrays', function () {
      ...
      Clazz_defineMethod(
        ...
        function (bioShape) {
          ...
          if (this.ms.isJmolDataFrameForModel(bioShape.modelIndex)) {
            this.controlPoints = bioShape.bioPolymer.getControlPoints(
              true,
              0,
              false,
            );
          ...
          for (var i = this.monomerCount; --i >= 0; ) {
            if (
              (this.monomers[i].shapeVisibilityFlags & this.myVisibilityFlag) ==
                0 ||
              this.ms.isAtomHidden(this.leadAtomIndices[i]) ||
              (bsDeleted != null && bsDeleted.get(this.leadAtomIndices[i]))
            )
              continue;
          ...
          this.isNucleic = Clazz_instanceOf(
            bioShape.bioPolymer,
            JM.NucleicPolymer,
          );
          this.isPhosphorusOnly =
            !this.isNucleic &&
            Clazz_instanceOf(bioShape.bioPolymer, JM.PhosphorusPolymer);
          this.isCarbohydrate = Clazz_instanceOf(
            bioShape.bioPolymer,
            JM.CarbohydratePolymer,
          );
          this.haveControlPointScreens = false;
      ...
      Clazz_defineMethod(c$, 'setStructureTypes', function () {
        var types = (this.structureTypes = this.vwr.allocTempEnum(
          this.monomerCount + 1,
        ));
        for (var i = this.monomerCount; --i >= 0; )
          if (
            ...
            types[i] = J.c.STR.NONE;

        types[this.monomerCount] = types[this.monomerCount - 1];
      });
      Clazz_defineMethod(c$, 'calcScreenControlPoints', function () {
      ...
      Clazz_defineMethod(
        ...
        function (i) {
          return JU.C.getColixInherited(
            this.colixes[i],
            this.monomers[i].getLeadAtom().colixAtom,
          );
        },
        '~N',
      );
      Clazz_defineMethod(
        ...
        'getLeadColixBack',
        function (i) {
          return this.colixesBack == null || this.colixesBack.length <= i
            ? 0
            : this.colixesBack[i];
        },
        '~N',
      ...
      Clazz_defineMethod(
        ...
        function (i, thisTypeOnly) {
          ...
          if (this.isTraceAlpha) {
            if (
              !thisTypeOnly ||
              this.structureTypes[i] === this.structureTypes[this.iNext]
            ) {
            ...
            if (
              !thisTypeOnly ||
              this.structureTypes[i] === this.structureTypes[this.iPrev]
            )
              this.madBeg =
                ((this.mads[this.iPrev] == 0
                  ? this.madMid
            ...
            if (
              ...
              this.madEnd =
                ((this.mads[this.iNext] == 0
                  ? this.madMid
                  : this.mads[this.iNext]) +
                  this.madMid) >>
                1;
          }
          this.diameterBeg = Clazz_floatToInt(
            this.vwr.tm.scaleToScreen(
              Clazz_floatToInt(this.controlPointScreens[i].z),
              this.madBeg,
            ),
          );
          this.diameterMid = Clazz_floatToInt(
            this.vwr.tm.scaleToScreen(
              this.monomers[i].getLeadAtom().sZ,
              this.madMid,
            ),
          );
          this.diameterEnd = Clazz_floatToInt(
            this.vwr.tm.scaleToScreen(
              Clazz_floatToInt(this.controlPointScreens[this.iNext].z),
              this.madEnd,
            ),
          );
          var doCap0 =
          ...
          return (
            this.aspectRatio > 0 &&
            this.meshRenderer != null &&
            this.meshRenderer.check(doCap0, doCap1)
          );
        },
        '~N,~B',
      );
      ...
      Clazz_defineMethod(
        ...
        function (i, thisTypeOnly, tension) {
          ...
          if (
            (this.diameterBeg == 0 && this.diameterEnd == 0) ||
            this.wireframeOnly
          )
            this.g3d.drawLineAB(
              this.controlPointScreens[i],
      ...
      Clazz_defineMethod(
        ...
        function (i) {
          ...
          this.calc1Screen(cp, wv, this.madBeg, 0.0007, this.screenArrowTop);
      ...
      Clazz_defineMethod(
        ...
        function (atomA, atomB, colixA, colixB, max) {
          ...
          if (mad < 0) {
            ...
            var width = Clazz_floatToInt(
              this.isExport
                ? mad
                : this.vwr.tm.scaleToScreen(
                    Clazz_doubleToInt((zA + zB) / 2),
                    mad,
                  ),
            );
            this.g3d.fillCylinderXYZ(
              colixA,
              colixB,
              3,
              width,
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'renderStrandShape', function () {
        if (!this.setStrandCount()) return;
        this.renderStrands();
      });
      Clazz_defineMethod(c$, 'setStrandCount', function () {
        if (this.wingVectors == null) return false;
        this.strandCount = Clazz_instanceOf(this.shape, J.shapebio.Strands)
          ? this.vwr.getStrandCount(this.shape.shapeID)
          : 10;
        this.strandSeparation =
          this.strandCount <= 1 ? 0 : 1 / (this.strandCount - 1);
        this.baseStrandOffset =
          (this.strandCount & 1) == 0
            ? this.strandSeparation / 2
            : this.strandSeparation;
        return true;
      });
      Clazz_defineMethod(c$, 'renderStrands', function () {
        var screens;
      ...
      Clazz_defineMethod(
        ...
        function (screens) {
          for (
            var i = this.bsVisible.nextSetBit(0);
            i >= 0;
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'renderMeshRibbon', function () {
        if (!this.setStrandCount()) return;
        var offset =
      ...
      Clazz_defineMethod(
        ...
        function (doFill, offsetTop, offsetBottom) {
          ...
          for (
            ...
            this.renderHermiteRibbon(doFill, i, false);
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'renderRockets', function () {
        if (this.rr == null)
      ...
      Clazz_defineMethod(c$, 'calcRopeMidPoints', function () {
        ...
        for (var i = 0; i <= this.monomerCount; ++i) {
          ...
          if (
            i < this.monomerCount &&
            ((this.helixRockets && this.structureTypes[i] === J.c.STR.HELIX) ||
              (this.isRockets && this.structureTypes[i] === J.c.STR.SHEET))
          ) {
            ...
            if (ptLastRocket == i - 1 && i > 1)
              this.cordMidPoints[i - 1].setT(
                this.cordMidPoints[i > 2 ? i - 3 : i - 2],
              );
            point.setT(
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'renderProtein', function () {
    ...
    function () {
      ...
      Clazz_overrideMethod(
        ...
        function (bioShape) {
          ...
          this.isDataFrame = this.ms.isJmolDataFrameForModel(
            bioShape.modelIndex,
          );
          ...
          for (
            ...
            if (
              atomA.nBackbonesDisplayed > 0 &&
              atomB.nBackbonesDisplayed > 0 &&
              !this.ms.isAtomHidden(atomB.i) &&
              (this.isDataFrame || atomA.distanceSquared(atomB) < 100)
            ) {
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'renderTrace', function () {
        this.calcScreenControlPoints();
        for (
          var i = this.bsVisible.nextSetBit(0);
          i >= 0;
          ...
          this.renderHermiteConic(i, false, 7);
      });
    },
  );
})(
