    ...
    function () {
      ...
      Clazz_overrideMethod(
        ...
        function (type, width, height) {
          ...
          var s = new JU.SB();
          var sfunc = isAll
            ? new JU.SB().append('function _setState() {\n')
            : null;
          if (isAll) {
            s.append(
          ...
          if (sfunc != null) {
            ...
            this.app(
              sfunc,
              'set antialiasTranslucent ' + global.antialiasTranslucent,
            );
      ...
      Clazz_defineMethod(
        ...
        function (sfunc) {
          ...
          if (this.vwr.userVdws != null) {
            var info = this.vwr.getDefaultVdwNameOrData(
              0,
              J.c.VDW.USER,
              this.vwr.bsUserVdws,
            );
        ...
        function (sfunc, isAll, withProteinStructure) {
            ...
            for (var i = 0; i < ms.bondCount; i++)
              if (!models[bonds[i].atom1.mi].isModelKit)
                if (bonds[i].isHydrogen() || (bonds[i].order & 131072) != 0) {
                  ...
                  sb.appendI(index)
                    .appendC('\t')
                    .appendI(bond.atom2.i)
                    .appendC('\t')
                    .appendI(bond.order & 131071)
                    .appendC('\t')
                    .appendF(bond.mad / 1000)
                    .appendC('\t')
                    .appendF(bond.getEnergy())
                    .appendC('\t')
                    .append(JU.Edge.getBondOrderNameFromOrder(bond.order))
                    .append(';\n');
                }
            if (sb.length() > 0)
              commands
          ...
          if (ms.haveHiddenBonds) {
            ...
            for (var i = ms.bondCount; --i >= 0; )
              if (
                bonds[i].mad != 0 &&
                (bonds[i].shapeVisibilityFlags & JM.Bond.myVisibilityFlag) == 0
              )
                bs.set(i);
          ...
          for (var i = 0; i < modelCount; i++)
            if (models[i].mat4 != null)
              commands.append(
                '  frame orientation ' +
                  ms.getModelNumberDotted(i) +
                  JU.Escape.matrixToScript(models[i].mat4) +
                  ';\n',
              );
          ...
          if (isAll) {
            ...
            for (var i = 0; i < modelCount; i++) {
              ...
              if (s != null && !s.equals(ms.getInfo(i, 'modelID0')))
                sb.append('  frame ID ').append(JU.PT.esc(s)).append(';\n');
              var t = ms.frameTitles[i];
              if (t != null && t.length > 0)
                sb.append('  frame title ').append(JU.PT.esc(t)).append(';\n');
              if (
                needOrientations &&
                m.orientation != null &&
                ...
                sb.append('  ')
                  .append(m.orientation.getMoveToText(false))
                  .append(';\n');
              if (m.frameDelay != 0 && !ms.isTrajectorySubFrame(i))
                sb.append('  frame delay ')
                  .appendF(m.frameDelay / 1000)
                  .append(';\n');
              if (m.simpleCage != null) {
                sb.append('  unitcell ')
                  .append(JU.Escape.eAP(m.simpleCage.getUnitCellVectors()))
                  .append(';\n');
                this.getShapeStatePriv(sb, isAll, 33);
              }
              if (sb.length() > 0)
                commands
                  .append('  frame ' + ms.getModelNumberDotted(i) + ';\n')
                  .appendSB(sb);
            }
            var loadUC = false;
            if (ms.unitCells != null) {
              ...
              for (var i = 0; i < modelCount; i++) {
                ...
                haveModulation = new Boolean(
                  haveModulation |
                    (this.vwr.ms.getLastVibrationVector(i, 1275072532) >= 0),
                ).valueOf();
              }
              ...
              if (haveModulation) {
                ...
                for (var i = modelCount; --i >= 0; ) {
                  if (
                    (ivib = this.vwr.ms.getLastVibrationVector(
                      i,
                      1275072532,
                    )) >= 0
                  )
                    for (var j = models[i].firstAtomIndex; j <= ivib; j++) {
                      var mset = ms.getModulation(j);
                      if (mset != null)
                        JU.BSUtil.setMapBitSet(temp, j, j, mset.getState());
                    }
                }
      ...
      Clazz_defineMethod(
        ...
        function (sfunc, width, height) {
          ...
          str
            .append('# width ')
            .appendI(width == 0 ? this.vwr.getScreenWidth() : width)
            .append(';\n# height ')
            .appendI(height == 0 ? this.vwr.getScreenHeight() : height)
            .append(';\n');
          this.app(str, 'stateVersion = ' + JV.JC.versionInt);
          this.app(
            str,
            'background ' + JU.Escape.escapeColor(global.objColors[0]),
          );
          for (var i = 1; i < 7; i++)
            if (global.objColors[i] != 0)
              this.app(
                str,
                JV.StateManager.getObjectNameFromId(i) +
                  'Color = "' +
                  JU.Escape.escapeColor(global.objColors[i]) +
                  '"',
              );
          ...
          if (global.backgroundImageFileName != null) {
            this.app(
              str,
              'background IMAGE ' +
                (global.backgroundImageFileName.startsWith(';base64,')
                  ? ''
                  : '/*file*/') +
                JU.PT.esc(global.backgroundImageFileName),
            );
          }
      ...
      Clazz_overrideMethod(
        ...
        function (isAll) {
          ...
          this.app(
            str,
            Math.pow(2, se) == pe
              ? 'set specularExponent ' + se
              : 'set phongExponent ' + pe,
          );
      ...
      Clazz_defineMethod(
        ...
        function (cmds) {
          ...
          if (ligandModelSet != null) {
            for (
              var key, $key = ligandModelSet.keySet().iterator();
          ...
          for (var i = 0; i < modelCount; i++) {
            ...
            if (models[i].isModelKit) {
              ...
              this.getInlineData(
                commands,
                this.vwr.getModelExtract(bs, false, true, 'MOL'),
                i > 0,
                null,
                null,
              );
              ...
              if (auxFiles != null) {
                for (var j = 0; j < auxFiles.size(); j++)
                  commands.append(
                    ';#FILE1=' + JU.PT.esc(auxFiles.get(j)) + ';',
                  );
              }
            }
          }
      ...
      Clazz_overrideMethod(
        ...
        function (
          ...
          var tag =
            (isAppend
              ? 'append' +
                (appendToModelIndex != null &&
                appendToModelIndex.intValue() != this.vwr.ms.mc - 1
                  ? ' modelindex=' + appendToModelIndex
                  : '')
              : 'model') + ' inline';
          loadScript
            .append('load /*data*/ data "')
            .append(tag)
            .append('"\n')
            .append(strModel)
            .append('end "')
            .append(tag)
            .append(
              loadFilter == null || loadFilter.length == 0
                ? ''
                : ' filter' + JU.PT.esc(loadFilter),
            )
            .append('";');
        },
        'JU.SB,~S,~B,Integer,~S',
      ...
      Clazz_defineMethod(
        ...
        function (p, s) {
          var n = 0;
          for (
            var entry, $entry = p.schemes.entrySet().iterator();
            ...
            if (name.length > 0 && !name.startsWith('=') && n++ >= 0)
              s.append(
                'color "' +
                  name +
                  '=' +
                  JU.ColorEncoder.getColorSchemeList(entry.getValue()) +
                  '";\n',
              );
          }
          return n;
      ...
      Clazz_defineMethod(
        ...
        function (am, sfunc) {
          ...
          commands
            ...
            .append(';\n# last ')
            .append(this.vwr.getModelNumberDotted(modelCount - 1))
            .append(';\n');
          if (am.backgroundModelIndex >= 0)
            this.app(
              commands,
              'set backgroundModel ' +
                this.vwr.getModelNumberDotted(am.backgroundModelIndex),
            );
            ...
            for (var i = modelCount; --i >= 0; ) {
              ...
              if (t != null)
                this.app(
                  commands,
                  'frame ' +
                    this.vwr.ms.getModelNumberDotted(i) +
                    ' align ' +
                    t,
                );
            }
          ...
          this.app(
            commands,
            'frame RANGE ' +
              am.getModelSpecial(-1) +
              ' ' +
              am.getModelSpecial(1),
          );
          this.app(
            commands,
            'animation DIRECTION ' + (am.animationDirection == 1 ? '+1' : '-1'),
          );
          this.app(commands, 'animation FPS ' + am.animationFps);
          this.app(
            commands,
            'animation MODE ' +
              JS.T.nameOf(am.animationReplayMode) +
              ' ' +
              am.firstFrameDelay +
              ' ' +
              am.lastFrameDelay,
          ...
          if (am.animationFrames != null) {
            this.app(
              commands,
              'anim frames ' + JU.Escape.eAI(am.animationFrames),
            );
            var i = am.caf;
      ...
      Clazz_defineMethod(
        ...
        function (global, sfunc) {
          var list = new Array(
            global.htBooleanParameterFlags.size() +
          ...
          var n = 0;
          for (
            var key, $key = global.htBooleanParameterFlags.keySet().iterator();
            $key.hasNext() && ((key = $key.next()) || true);

          )
            if (JV.GlobalSettings.doReportProperty(key))
              list[n++] =
                'set ' + key + ' ' + global.htBooleanParameterFlags.get(key);

          for (
            var key,
              $key = global.htNonbooleanParameterValues.keySet().iterator();
            $key.hasNext() && ((key = $key.next()) || true);
          ...
          for (var i = 0; i < n; i++)
            if (list[i] != null) this.app(commands, list[i]);

          var s = JV.StateManager.getVariableList(
            global.htUserVariables,
            0,
            false,
            true,
          );
          ...
          if (this.vwr.shm.getShape(5) != null)
            commands.append(this.getDefaultLabelState(this.vwr.shm.shapes[5]));
          if (global.haveSetStructureList) {
            var slist = global.structureList;
            commands.append(
              'struture HELIX set ' + JU.Escape.eAF(slist.get(J.c.STR.HELIX)),
            );
            commands.append(
              'struture SHEET set ' + JU.Escape.eAF(slist.get(J.c.STR.SHEET)),
            );
            commands.append(
              'struture TURN set ' + JU.Escape.eAF(slist.get(J.c.STR.TURN)),
            );
          }
          if (sfunc != null) commands.append('\n}\n\n');
          return commands.toString();
        ...
        function (l) {
          ...
          this.app(
            ...
            J.shape.Shape.getColorCommand(
              ...
              l.defaultPaletteID,
              l.defaultColix,
              l.translucentAllowed,
            ),
          );
          this.app(
            s,
            'background label ' + J.shape.Shape.encodeColor(l.defaultBgcolix),
          );
          this.app(
            s,
            'set labelOffset ' +
              JV.JC.getXOffset(l.defaultOffset) +
              ' ' +
              JV.JC.getYOffset(l.defaultOffset),
          );
          var align = JV.JC.getHorizAlignmentName(l.defaultAlignment);
          this.app(
            s,
            'set labelAlignment ' + (align.length < 5 ? 'left' : align),
          );
          var pointer = JV.JC.getPointerName(l.defaultPointer);
          this.app(
            s,
            'set labelPointer ' + (pointer.length == 0 ? 'off' : pointer),
          );
          ...
          this.app(
            ...
            J.shape.Shape.getFontCommand(
              'label',
              JU.Font.getFont3D(l.defaultFontId),
            ),
          );
          return s.toString();
        },
      ...
      Clazz_defineMethod(
        ...
        function (tm, sfunc) {
          ...
          this.app(
            commands,
            'set scaleAngstromsPerInch ' + tm.scale3DAngstromsPerInch,
          );
          ...
          commands.append(
            this.vwr.getOrientation(1073742034, null, null).toString(),
          );
          ...
          commands
            ...
            .append(tm.slabEnabled && !navigating ? ';slab on' : '')
            .append(';\n');
          commands
            .append('  set slabRange ')
            .appendF(tm.slabRange)
            .append(';\n');
          if (tm.slabPlane != null)
            commands
              .append('  slab plane ')
              .append(JU.Escape.eP4(tm.slabPlane))
              .append(';\n');
          if (tm.depthPlane != null)
            commands
              .append('  depth plane ')
              .append(JU.Escape.eP4(tm.depthPlane))
              .append(';\n');
          ...
          if (this.vwr.ms.modelSetHasVibrationVectors() && tm.vibrationOn)
            this.app(
              commands,
              'set vibrationPeriod ' + tm.vibrationPeriod + ';vibration on',
            );
        ...
        function (s, tm, isAll) {
          ...
          try {
            if (tm.zSlabPoint != null)
              this.app(s, 'set zSlab ' + JU.Escape.eP(tm.zSlabPoint));
          } catch (e) {
      ...
      Clazz_overrideMethod(
        ...
        function (isAll) {
          ...
          var s =
            '  set spinX ' +
            Clazz_floatToInt(tm.spinX) +
            '; set spinY ' +
            Clazz_floatToInt(tm.spinY) +
            '; set spinZ ' +
            Clazz_floatToInt(tm.spinZ) +
            '; set spinFps ' +
            Clazz_floatToInt(tm.spinFps) +
            ';';
          if (!Float.isNaN(tm.navFps))
            s +=
              '  set navX ' +
              Clazz_floatToInt(tm.navX) +
              '; set navY ' +
              Clazz_floatToInt(tm.navY) +
              '; set navZ ' +
              Clazz_floatToInt(tm.navZ) +
              '; set navFps ' +
              Clazz_floatToInt(tm.navFps) +
              ';';
          ...
          if (tm.isSpinInternal) {
            ...
            s +=
              ...
              tm.rotationRate +
              ' ' +
              JU.Escape.eP(tm.internalRotationCenter) +
              ' ' +
              JU.Escape.eP(pt);
      ...
      Clazz_defineMethod(
        ...
        function (myType, font3d) {
          var objId = JV.StateManager.getObjectIdFromName(
            myType.equalsIgnoreCase('axes') ? 'axis' : myType,
          );
          if (objId < 0) return '';
      ...
      c$.addTickInfo = Clazz_defineMethod(
        c$,
        'addTickInfo',
        function (sb, tickInfo, addFirst) {
          sb.append(' ticks ')
            .append(tickInfo.type)
            .append(' ')
            .append(JU.Escape.eP(tickInfo.ticks));
          var isUnitCell =
            tickInfo.scale != null && Float.isNaN(tickInfo.scale.x);
          if (isUnitCell) sb.append(' UNITCELL');
          if (tickInfo.tickLabelFormats != null)
            sb.append(' format ').append(
              JU.Escape.eAS(tickInfo.tickLabelFormats, false),
            );
          if (!isUnitCell && tickInfo.scale != null)
            sb.append(' scale ').append(JU.Escape.eP(tickInfo.scale));
          if (addFirst && !Float.isNaN(tickInfo.first) && tickInfo.first != 0)
            sb.append(' first ').appendF(tickInfo.first);
          if (tickInfo.reference != null)
            sb.append(' point ').append(JU.Escape.eP(tickInfo.reference));
        },
        'JU.SB,JM.TickInfo,~B',
      );
      Clazz_defineMethod(
        ...
        function (shape) {
          ...
          for (var i = 0; i < measurementCount; i++) {
            ...
            if (m.mad != 0)
              sb.append(' radius ').appendF(
                m.thisID == null || m.mad > 0 ? m.mad / 2000 : 0,
              );
            if (m.colix != 0)
              sb.append(' color ').append(
                JU.Escape.escapeColor(JU.C.getArgb(m.colix)),
              );
            if (m.text != null) {
              if (m.text.font != null)
              ...
              if (m.text.align != 0)
                sb.append(' align ').append(
                  JV.JC.getHorizAlignmentName(m.text.align),
                );
              if (m.text.pymolOffset != null)
                sb.append(' offset ').append(JU.Escape.eAF(m.text.pymolOffset));
            }
            var tickInfo = m.tickInfo;
            if (tickInfo != null)
              JV.StateCreator.addTickInfo(sb, tickInfo, true);
            ...
            if (isProperty) {
              sb.append(
                ' ' +
                  m.property +
                  ' value ' +
                  (Double.isNaN(m.value) ? 0 : m.value),
              ).append(' ' + JU.PT.esc(m.getString()));
            } else if (count == 2) {
          ...
          if (nHidden > 0)
            if (nHidden == measurementCount)
              ...
              for (var i = 0; i < measurementCount; i++)
                if (bs.get(i))
                  JU.BSUtil.setMapBitSet(temp, i, i, 'measure off');
        ...
        function (shape) {
          ...
          if (reportAll || shape.bsSizeSet != null) {
            ...
            for (
              ...
              JU.BSUtil.setMapBitSet(
                ...
                i,
                'wireframe ' +
                  ((r = bonds[i].mad) == 1 ? 'on' : '' + JU.PT.escF(r / 2000)),
              );
          }
          if (reportAll || bsOrderSet != null) {
            ...
            for (
              ...
              var bond = bonds[i];
              if (reportAll || (bond.order & 131072) == 0)
                JU.BSUtil.setMapBitSet(
                  this.temp,
                  i,
                  i,
                  'bondOrder ' + JU.Edge.getBondOrderNameFromOrder(bond.order),
                );
            }
          }
          if (shape.bsColixSet != null)
            for (
              var i = shape.bsColixSet.nextSetBit(0);
              i >= 0;
          ...
          var s =
            this.getCommands(this.temp, null, 'select BONDS') +
            '\n' +
            (haveTainted
              ? this.getCommands(this.temp2, null, 'select BONDS') + '\n'
              : '');
          this.clearTemp();
          return s;
        },
      ...
      Clazz_defineMethod(c$, 'clearTemp', function () {
        this.temp.clear();
        this.temp2.clear();
      });
      Clazz_defineMethod(
        ...
        function (shape) {
          ...
          switch (shape.shapeID) {
            ...
            case 31:
              ...
              for (
                var t, $t = es.objects.values().iterator();
            ...
            case 35:
                ...
                for (var i = this.vwr.ms.ac; --i >= 0; ) {
                  ...
                  if (h.atomFormats[i] != null) {
                    JU.BSUtil.setMapBitSet(
                      ...
                      i,
                      i,
                      'set hoverLabel ' + JU.PT.esc(h.atomFormats[i]),
                    );
                  }
                }
              }
              s =
                '\n  hover ' +
                JU.PT.esc(h.labelFormat == null ? '' : h.labelFormat) +
                ';\n' +
            ...
            case 5:
              ...
              this.clearTemp();
              for (
                var i = l.bsSizeSet.nextSetBit(0);
                i >= 0;
                ...
                if (t == null) {
                  ...
                  if (t.pymolOffset != null)
                    cmd += ';set labelOffset ' + JU.Escape.eAF(t.pymolOffset);
                }
                ...
                if (l.bsBgColixSet != null && l.bsBgColixSet.get(i))
                  JU.BSUtil.setMapBitSet(
                    ...
                    i,
                    'background label ' +
                      J.shape.Shape.encodeColor(l.bgcolixes[i]),
                  );
                ...
                if (sppm > 0)
                  JU.BSUtil.setMapBitSet(
                    ...
                    i,
                    i,
                    'set labelScaleReference ' + 10000 / sppm,
                  );
                if (l.offsets != null && l.offsets.length > i) {
                  ...
                  JU.BSUtil.setMapBitSet(
                    ...
                    i,
                    'set ' +
                      (JV.JC.isOffsetAbsolute(offsetFull)
                        ? 'labelOffsetAbsolute '
                        : 'labelOffset ') +
                      JV.JC.getXOffset(offsetFull) +
                      ' ' +
                      JV.JC.getYOffset(offsetFull),
                  );
                  ...
                  if (pointer.length > 0)
                    JU.BSUtil.setMapBitSet(
                      ...
                      i,
                      i,
                      'set labelPointer ' + pointer,
                    );
                  if ((offsetFull & 32) != 0)
                    JU.BSUtil.setMapBitSet(this.temp2, i, i, 'set labelFront');
                  else if ((offsetFull & 16) != 0)
                    JU.BSUtil.setMapBitSet(this.temp2, i, i, 'set labelGroup');
                  if (align.length > 0)
                    JU.BSUtil.setMapBitSet(
                      this.temp3,
                      i,
                      i,
                      'set labelAlignment ' + align,
                    );
                }
                if (l.mads != null && l.mads[i] < 0)
                  JU.BSUtil.setMapBitSet(this.temp2, i, i, 'set toggleLabel');
              ...
              for (var i = 0; i < ac; i++) {
                ...
                if (shape.bsSizeSet != null && shape.bsSizeSet.get(i)) {
                  if ((r = atoms[i].madAtom) < 0)
                    JU.BSUtil.setMapBitSet(this.temp, i, i, 'Spacefill on');
                  else
                    JU.BSUtil.setMapBitSet(
                      this.temp,
                      i,
                      i,
                      'Spacefill ' + JU.PT.escF(r / 2000),
                    );
                }
                if (shape.bsColixSet != null && shape.bsColixSet.get(i)) {
                  ...
                  if (
                    pid != J.c.PAL.CPK.id ||
                    JU.C.isColixTranslucent(atoms[i].colixAtom)
                  )
                    JU.BSUtil.setMapBitSet(
                      this.temp,
                      i,
      ...
      Clazz_overrideMethod(
        ...
        function (shape) {
          ...
          if (shape.bsSizeSet != null)
            for (
              var i = shape.bsSizeSet.nextSetBit(0);
              i >= 0;
      ...
      Clazz_defineMethod(
        ...
        function (sb, t) {
          ...
          switch (t.valign) {
            case 3:
              if (
                t.movableXPercent == 2147483647 ||
                t.movableYPercent == 2147483647
              ) {
                strOff =
                  (t.movableXPercent == 2147483647
                    ? t.movableX + ' '
                ...
                strOff =
                  '[' + t.movableXPercent + ' ' + t.movableYPercent + '%]';
              }
            case 4:
              ...
              if (t.align != 4)
                sb.append(';  ')
                  .append(echoCmd)
                  .append(' ')
                  .append(JV.JC.getHorizAlignmentName(t.align));
              break;
            default:
              sb.append('  set echo ')
                .append(JV.JC.getEchoName(t.valign))
                .append(' ')
                .append(JV.JC.getHorizAlignmentName(t.align));
          }
          ...
          if (isImage && t.imageScale != 1)
            sb.append('  ')
              .append(echoCmd)
              .append(' scale ')
              .appendF(t.imageScale)
              .append(';\n');
          if (t.script != null)
            sb.append('  ')
              .append(echoCmd)
              .append(' script ')
              .append(JU.PT.esc(t.script))
              .append(';\n');
          if (t.modelIndex >= 0)
            sb.append('  ')
              .append(echoCmd)
              .append(' model ')
              .append(this.vwr.getModelNumberDotted(t.modelIndex))
              .append(';\n');
          if (t.pointerPt != null) {
            sb.append('  ')
              .append(echoCmd)
              .append(' point ')
              .append(
                Clazz_instanceOf(t.pointerPt, JM.Atom)
                  ? '({' + t.pointerPt.i + '})'
                  : JU.Escape.eP(t.pointerPt),
              )
              .append(';\n');
          }
          if (t.pymolOffset != null) {
            sb.append('  ')
              .append(echoCmd)
              .append(' offset ')
              .append(JU.Escape.escapeFloatA(t.pymolOffset, true))
              .append(';\n');
          }
          t.appendFontCmd(sb);
          sb.append('; color echo');
          if (JU.C.isColixTranslucent(t.colix))
            sb.append(' ').append(JU.C.getColixTranslucencyLabel(t.colix));
          sb.append(' ').append(JU.C.getHexCode(t.colix));
          if (t.bgcolix != 0) {
            sb.append('; color echo background ');
            if (JU.C.isColixTranslucent(t.bgcolix))
              sb.append(JU.C.getColixTranslucencyLabel(t.bgcolix)).append(' ');
          ...
          if (t.hidden) {
            if (isScale) sb.append('  set echo "%SCALE" hidden;\n');
              ...
              sb.append('  set echo ID ')
                .append(JU.PT.esc(t.target))
                .append(' hidden;\n');
          }
        },
        'JU.SB,JM.Text',
      ...
      Clazz_overrideMethod(
        ...
        function (prefix) {
          ...
          var commands = new JU.SB();
          var list = new Array(
            g.htBooleanParameterFlags.size() +
              g.htNonbooleanParameterValues.size() +
              g.htUserVariables.size(),
          );
          var n = 0;
          var _prefix = '_' + prefix;
          for (
            var key, $key = g.htBooleanParameterFlags.keySet().iterator();
            $key.hasNext() && ((key = $key.next()) || true);
            ...
            if (
              ...
              list[n++] =
                (key.indexOf('_') == 0 ? key + ' = ' : 'set ' + key + ' ') +
                g.htBooleanParameterFlags.get(key);
          }
          for (
            var key, $key = g.htNonbooleanParameterValues.keySet().iterator();
            $key.hasNext() && ((key = $key.next()) || true);
            ...
            if (
              ...
              if (Clazz_instanceOf(value, String))
                value = JV.StateCreator.chop(JU.PT.esc(value));
              list[n++] =
                (key.indexOf('_') == 0 ? key + ' = ' : 'set ' + key + ' ') +
                value;
            }
          ...
          for (
            var key, $key = g.htUserVariables.keySet().iterator();
            $key.hasNext() && ((key = $key.next()) || true);
            ...
            if (prefix == null || key.indexOf(prefix) == 0) {
              ...
              list[n++] =
                ...
                ' ' +
                (key.startsWith('@') ? '' : '= ') +
                (value.tok == 4 ? JV.StateCreator.chop(JU.PT.esc(s)) : s);
            }
          }
          java.util.Arrays.sort(list, 0, n);
          for (var i = 0; i < n; i++)
      ...
      Clazz_overrideMethod(
        ...
        function (f) {
          ...
          var namesOnly =
            f.equalsIgnoreCase('names') || f.equalsIgnoreCase('static_names');
          if (namesOnly) f = '';
      ...
      Clazz_defineMethod(
        ...
        function (s, ht, selectedFunction, isGeneric, namesOnly) {
          ...
          var n = 0;
          for (
            var name, $name = ht.keySet().iterator();
            $name.hasNext() && ((name = $name.next()) || true);

          )
            if (
              (selectedFunction.length == 0 && !name.startsWith('_')) ||
              name.equalsIgnoreCase(selectedFunction) ||
      ...
      c$.isTainted = Clazz_defineMethod(
        c$,
        'isTainted',
      ...
      Clazz_overrideMethod(
        ...
        function (taintWhat, bsSelected) {
          var bs;
          var commands = new JU.SB();
          for (var type = 0; type < 18; type++)
            if (taintWhat < 0 || type == taintWhat)
              if (
                (bs =
                  bsSelected != null
                    ? bsSelected
                    : this.vwr.ms.getTaintedAtoms(type)) != null
              )
                this.getAtomicPropertyStateBuffer(
                  commands,
                  type,
                  bs,
                  null,
      ...
      Clazz_overrideMethod(
        ...
        function (commands, type, bs, label, fData) {
          ...
          var dataLabel =
            (label == null
              ? JM.AtomCollection.userSettableValues[type]
              : label) + ' set';
          var n = 0;
          ...
          if (bs != null)
            for (var i = bs.nextSetBit(0); i >= 0; i = bs.nextSetBit(i + 1)) {
              ...
              s.appendI(i + 1)
                .append(' ')
                .append(atoms[i].getElementSymbol())
                .append(' ')
                .append(atoms[i].getInfo().$replace(' ', '_'))
                .append(' ');
              switch (type) {
                case 18:
                ...
                case 1:
                  ...
                  break;
                case 2:
                  if (JV.StateCreator.isTainted(tainted, i, 2))
                    isDefault = false;
                  s.appendF(atoms[i].x)
                    .append(' ')
                    .appendF(atoms[i].y)
                ...
                case 11:
                  s.appendF(
                    atoms[i].getVanderwaalsRadiusFloat(this.vwr, J.c.VDW.AUTO),
                  );
                  break;
              }
              s.append(' ;\n');
          ...
          switch (action) {
            ...
            case 4140:
              switch (n) {
                case 1275068425:
                  return (
                    action == 4165 ? this.actionStates : this.actionStatesRedo
                  ).size();
                case -2:
        ...
        function (taintedAtom, type, clearRedo) {
          ...
          switch (type) {
            ...
            case 4165:
              ...
              if (
                this.vwr.ms.am[modelIndex].isModelKit ||
                s.indexOf('zap ') < 0
              ) {
            ...
            default:
              ...
              if (taintedAtom >= 0) {
                ...
                sb.append('set refreshing false;')
                  ...
                  .append(this.vwr.tm.getMoveToText(0, false))
      ...
      Clazz_defineStatics(c$, 'MAX_ACTION_UNDO', 100);
    },
  );
})(
  Clazz,
