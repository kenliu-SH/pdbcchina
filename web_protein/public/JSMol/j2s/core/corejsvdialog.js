(function (
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(c$, 'getPosXY', function () {
        return JSV.dialog.IntegrationDialog.posXY;
      });
      Clazz_defineMethod(c$, 'addUniqueControls', function () {
      ...
      Clazz_overrideMethod(c$, 'applyFromFields', function () {
        this.apply(
          Clazz_newArray(-1, [
        ...
        function (id, msg) {
          ...
          try {
            if (id.equals('SHOWSELECTION')) {
              for (var i = 0; i < this.xyData.size(); i++)
                if (
                  JU.DF.formatDecimalDbl(
                    this.xyData.get(i).getXVal(),
                    2,
                  ).equals(msg)
                ) {
                  this.iSelected = i;
            ...
            if (id.equals('btnDelete')) {
              ...
              var ret = this.manager.getDialogInput(
                this.dialog,
      ...
      Clazz_defineMethod(c$, 'checkSelectedIntegral', function () {
        if (this.iSelected < 0) {
          this.showMessage(
      ...
      Clazz_defineMethod(c$, 'deleteIntegral', function () {
      ...
      Clazz_defineStatics(c$, 'posXY', Clazz_newIntArray(-1, [-2147483648, 0]));
    },
  );
  Clazz_declarePackage('JSV.dialog');
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'addUniqueControls', function () {
        ...
        this.combo1 = this.dialog.addSelectOption(
          'cmbInterpolation',
          'Interpolation',
          Clazz_newArray(-1, ['parabolic', 'none']),
          0,
          true,
        );
      });
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(
        ...
        function (rootNode, level, isViews) {
          ...
          while (enume.hasMoreElements()) {
            ...
            var cb = this.dialog.addCheckBox(
              name,
              title,
              level,
              node.isSelected,
      ...
      Clazz_overrideMethod(c$, 'checkEnables', function () {
        var n = 0;
        for (var i = 0; i < this.checkBoxes.size(); i++) {
          if (
            this.dialog.isSelected(this.checkBoxes.get(i)) &&
            this.treeNodes.get(i).getPanelNode().jsvp != null
          ) {
            n++;
          }
        }
        ...
        function (name) {
          ...
          if (node.getPanelNode().jsvp == null) {
            if (
              !this.checking &&
              isSelected &&
              this.dialog.getText(cb).startsWith('Overlay')
            ) {
              this.checking = true;
            ...
            while (enume.hasMoreElements()) {
              ...
              this.dialog.setSelected(
                this.checkBoxes.get(treeNode.getIndex()),
                isSelected,
              );
          ...
          if (isSelected)
            for (i = this.treeNodes.size(); --i >= 0; )
              if (
                this.treeNodes.get(i).getPanelNode().isView !=
                node.getPanelNode().isView
              ) {
                this.dialog.setSelected(
                  this.checkBoxes.get(this.treeNodes.get(i).getIndex()),
                  false,
                );
      ...
      Clazz_defineMethod(c$, 'combineSelected', function () {});
      Clazz_defineMethod(c$, 'viewSelected', function () {
        var sb = new JU.SB();
        var thisNode = null;
        var n = 0;
        for (var i = 0; i < this.checkBoxes.size(); i++) {
      ...
      Clazz_defineMethod(c$, 'closeSelected', function () {
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_overrideMethod(
        ...
        'getDialog',
        function (jsvDialog) {
          return new JSV.js2d.JsDialog(
            this,
            jsvDialog,
            this.registerDialog(jsvDialog),
          );
        },
        'JSV.dialog.JSVDialog',
      );
      ...
      Clazz_overrideMethod(
        ...
        function (frame, spectrum) {
          ...
          var tableModel = new JSV.js2d.DialogTableModel(
            columnNames,
            rowData,
            false,
            true,
  ...
  Clazz_load(
    ...
    function () {
        ...
        function (frame, currentSource) {
          ...
          if (errorLog != null && errorLog.length > 0)
            this.showMessage(
              frame,
              errorLog,
              JSV.dialog.DialogManager.fixTitle(currentSource.getFilePath()),
            );
          else
        ...
        function (frame, spec) {
          ...
          try {
            ...
            this.showMessage(
              null,
              s,
              JSV.dialog.DialogManager.fixTitle(filePath),
            );
          } catch (ex) {
            if (Clazz_exceptionOf(ex, Exception)) {
      ...
      Clazz_defineMethod(c$, 'getDialogOptions', function () {
        if (this.options == null) this.options = new java.util.Hashtable();
        return this.options;
      });
      c$.fixTitle = Clazz_defineMethod(
        c$,
        'fixTitle',
        function (title) {
        ...
        function (columnNames, data, asString, tableCellAlignLeft) {
          ...
          this.widths =
            data.length == 0
              ...
              : Clazz_newIntArray(data[0].length, 0);
          this.tableCellAlignLeft = tableCellAlignLeft;
        },
        '~A,~A,~B,~B',
      );
      Clazz_defineMethod(c$, 'getColumnCount', function () {
  ...
  Clazz_load(
    ...
    function () {
      ...
      Clazz_defineMethod(c$, 'onFocus', function () {
        this.toFront();
      });
      Clazz_overrideMethod(
        c$,
      ...
      Clazz_defineMethod(
        ...
        function (name, label, obj, units) {
          this.thisPanel.add(
            new JS.JLabel(label == null ? name : label),
            new JS.GridBagConstraints(
              0,
              this.iRow,
              1,
      ...
      Clazz_overrideMethod(
        ...
        function (data, header, widths) {
          try {
            var scrollPane = new JS.JScrollPane(
      ...
      Clazz_overrideMethod(c$, 'endLayout', function () {
      ...
      Clazz_overrideMethod(c$, 'startLayout', function () {
        ...
        if (this.haveTwoPanels) {
          this.thisPanel = this.leftPanel = new JS.JPanel(
            new JS.GridBagLayout(),
          );
