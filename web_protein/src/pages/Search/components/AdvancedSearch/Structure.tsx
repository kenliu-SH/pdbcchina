...
export default ({ structure, setStructure }: any) => {
  const { queryParams } = useConnect(NSearch);
  const { selectorItems } = queryParams?.structure || [];
  ...
  const onRemove = (index: number) => {
    structure.nodes.splice(index, 1);
    setStructure(Object.assign({}, structure));
  };
  //AND 或 OR
  const onOperator = () => {
    const { logical_operator } = structure;
    structure.logical_operator = logical_operator === 'and' ? 'or' : 'and';
    setStructure(Object.assign({}, structure));
  };
  //选择检索项
  const onSelect = (node: any, index: number) => {
    const { uiAttrMap } = queryParams['structure'] || {};
    if (node && uiAttrMap) {
      const { nested_attribute, attribute } = node;
      const vo: any =
        uiAttrMap[nested_attribute ? nested_attribute : attribute];
      structure.nodes[index].nodes[0].uiAttrMap = vo;
      structure.nodes[index].nodes[0].parameters = {
        ...node,
        negation: false,
  ...
  const onSave = (value: any, index: number, fieldKey: any) => {
    structure.nodes[index].nodes[0].parameters[fieldKey] = value;
    setStructure(Object.assign({}, structure));
  };
  return (
    <>
      {structure.nodes?.map((item: any, index: number) => {
        const { nodes } = item;
        ...
        return (
          <div key={index} className="subquery_box">
            <div className="operator">
              {index > 0 && structure?.logical_operator?.toLocaleUpperCase()}
            </div>
